	...
	Для Каждого СтрокаТаблицы Из ЗагружаемыеДанные Цикл
			...
			Если ОбъектСопоставленияЗаполнен Тогда
				...
				Если ЭлементСправочника = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Номенклатура с артикулом %1 не существует.';
							|en = 'Product with product ID %1 does not exist.'"), СтрокаТаблицы.Артикул);
				КонецЕсли;
		...
		Исключение
			...
			СтрокаТаблицы.ОписаниеОшибки = НСтр("ru = 'Невозможна запись данных по причине:';
												|en = 'Couldn''t save the data due to:'") + Символы.ПС + Причина;
		КонецПопытки;
	КонецЦикла;
