...
Процедура ОбъектМетаданныхПриИзменении(Элемент)
	...
	ЗаписатьКопиюТабличногоДокумента(Ложь);
...
Процедура РежимФормыПриИзменении(Элемент)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаКод Тогда
		...
		РезультатПреобразования = ПреобразоватьВТаблицуНаСервере();
		Если Не РезультатПреобразования.Успешно Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(РезультатПреобразования.ТекстОшибки);
			РежимФормы = "СтраницаКод";
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура ВернутьсяКПоследнейРабочейВерсии(Команда)
	
	ВосстановитьТабличныйДокументИзКопии();
...
&НаКлиенте
Процедура Код1СПриИзменении(Элемент)
	
	Элементы.ВернутьсяКПоследнейРабочейВерсии.Доступность = Истина;
...
Процедура Подключаемый_ТабличныйДокументПриАктивизацииОбласти(Элемент)
	...
	ИмяТабличногоДокумента = ?(Элемент.Имя <> Элементы.ТабличныйДокументБезТЧ.Имя,
		Элемент.Имя, Элементы.ТабличныйДокумент.Имя);
	
	ВысотаТаблицы = ЭтотОбъект[ИмяТабличногоДокумента].ВысотаТаблицы;
	...
	НизОбласти   = ?(ИсходнаяОбласть.Низ   = 0, ВысотаТаблицы, ИсходнаяОбласть.Низ);
	ЛевоОбласти  = ?(ИсходнаяОбласть.Лево  = 0, 1, ИсходнаяОбласть.Лево);
	ПравоОбласти = ?(ИсходнаяОбласть.Право = 0, ШиринаТаблицы, ИсходнаяОбласть.Право);
	
	ИмяОбласти = ЭтотОбъект[ИмяТабличногоДокумента].Область(1, ЛевоОбласти, 1, ПравоОбласти).Имя;
	
	Элемент.Защита = (ВерхОбласти = 1) Или (ПравоОбласти > ШиринаТаблицы);
	...
	Если (ИмяТабличногоДокумента = "ТабличныйДокумент") И ОтображатьТабличныеЧасти
		 И СтрНайти(ИмяОбластиШапки, ИмяОбласти(КоординатыОбласти) + "C") = 0 Тогда
		КоординатыОбласти.Вставить("Верх",  ВерхОбласти);
...
Процедура Подключаемый_ТабличныйДокументНачалоВводаЗначения(Элемент, Область, СтандартнаяОбработка)
	...
	ЗаписатьКопиюТабличногоДокумента();
	...
	Если ЭтоВидСчета(ТипРеквизитаСтруктуры) Тогда
		Если ПустаяСтрока(Область.Текст) Тогда
			ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ТабличныйДокументВыборЗавершениеВыбораРеквизитаВидаСчета",
				ЭтотОбъект, ДополнительныеПараметры);
			ВидыСчета = Новый СписокЗначений;
			ВидыСчета.Добавить(ВидСчета.АктивноПассивный);
			ВидыСчета.Добавить(ВидСчета.Активный);
			ВидыСчета.Добавить(ВидСчета.Пассивный);
			
			ВидыСчета.ПоказатьВыборЭлемента(ОписаниеОповещенияОЗакрытии, НСтр("ru = 'Выбор вида счета';
																				|en = 'Select account type'"));
		КонецЕсли;
		
	ИначеЕсли ЭтоОписаниеТипов(ТипРеквизитаСтруктуры) Тогда
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ТабличныйДокументВыборЗавершениеВыбораРеквизитаОписаниеТипов",
			ЭтотОбъект, ДополнительныеПараметры);
		ОписаниеТипов = ТабличныеДанныеПрочитать(ТекущиеИмяПредопределенныхДанных, ИмяРеквизита, ИмяТабличнойЧасти,
		...
		ОткрытьФорму("ВнешняяОбработка.НачальноеЗаполнениеДанных.Форма.ФормаРедактированияОписанияТипов",
			ПараметрыОткрытия, ЭтотОбъект,,,, ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		МетаданныеРеквизитаПолноеИмя = ПолноеИмяМетаданныхТипа(ТипРеквизитаСтруктуры);
		
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ТабличныйДокументВыборЗавершениеВыбораРеквизитаСсылочногоТипа",
			ЭтотОбъект, ДополнительныеПараметры);
		
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("РежимВыбора",           Истина);
		ПараметрыОткрытия.Вставить("ВыборГруппИЭлементов",  ИспользованиеГруппИЭлементов.ГруппыИЭлементы);
		ПараметрыОткрытия.Вставить("ТипРеквизитаСтруктуры", ТипРеквизитаСтруктуры);
		
		СтрокаИмениФормы = ?((ИмяРеквизита = "Родитель") И (Не ЭтоПланСчетов(ТипРеквизитаСтруктуры)), ".ФормаВыбораГруппы", ".ФормаВыбора");
		
		ОткрытьФорму(МетаданныеРеквизитаПолноеИмя + СтрокаИмениФормы, ПараметрыОткрытия, ЭтотОбъект,,,,
...
Процедура Подключаемый_ТабличныйДокументПриИзменении(Элемент)
	...
	ЭтоИзменениеЯчейки    = (Не ЭтоИзменениеСтрок) И (Не ЭтоИзменениеКолонок) И (ТекущаяОбласть.Верх = ТекущаяОбласть.Низ)
	...
	Если ЭтоИзменениеЯчейки Или ЭтоИзменениеЗаголовка Тогда
		...
		Если СтрНайти(ИмяРеквизита, "_") > 0 Тогда
			ЧастиИмени = СтрРазделить(ИмяРеквизита, "_");
			ИмяРеквизита = ЧастиИмени[0];
		КонецЕсли;
		...
		Если ЗначениеРеквизитаСтруктуры = Неопределено Тогда
			...
			Локализуемый = Ложь;
		Иначе
			ТипЗначения  = ЗначениеРеквизитаСтруктуры.ТипЗначения;
			Локализуемый = ЗначениеРеквизитаСтруктуры.Локализуемый;
		КонецЕсли;
		
		РезультатПроверки = РезультатПроверкиДанныхЯчейки(ТекущаяОбласть, ТипЗначения, ИмяРеквизита);
		Если РезультатПроверки.Успешно Тогда
			Если ЭтоСсылка(ТипЗнч(РезультатПроверки.Значение)) И (Не ЗначениеЗаполнено(РезультатПроверки.Значение)) Тогда
				ТекущаяОбласть.СодержитЗначение = Ложь;
			КонецЕсли;
			
			ТекущаяОбласть.Примечание.Текст = "";
			Если Не ЗначениеРеквизитаСтруктуры.Локализуемый Тогда
				ТекущаяОбласть.ЦветФона = WebЦвета.Белый;
			КонецЕсли;
			Если ЗначениеЗаполнено(РезультатПроверки.Значение) Тогда
				Если РезультатПроверки.ЭтоЗначение Тогда
					ТекущаяОбласть.Значение = РезультатПроверки.Значение;
				Иначе
					ТекущаяОбласть.Текст = РезультатПроверки.Значение;
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			...
			ТекущаяОбласть.Примечание.Текст = РезультатПроверки.ТекстОшибки;
			Если Не ЗначениеРеквизитаСтруктуры.Локализуемый Тогда
				ТекущаяОбласть.ЦветФона = WebЦвета.Розовый;
			КонецЕсли;
		КонецЕсли;
		...
		Если ИмяТабличногоДокумента = "ТабличныйДокумент" Тогда
			...
			Если ТекущиеИмяПредопределенныхДанныхКолонка = 0 Тогда
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Нельзя удалять обязательный реквизит с именем ключевого реквизита ""%1"".';
						|en = 'Cannot delete a mandatory attribute that contains the name of a key attribute ""%1"".'"), ИмяКлючевогоРеквизита));
	...
	Если ЭтоИзменениеЗаголовка Тогда
		...
		ТекстОшибки = "";
		Если ЗначениеРеквизитаСтруктуры = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У объекта метаданных нет реквизита ""%1"" %2';
																						|en = 'Metadata object does not have the ""%1"" attribute%2'"), ИмяРеквизита,
				?(ПустаяСтрока(ИмяРеквизитаТЧ), "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'табличной части ""%1""';
																												|en = 'table ""%1""'"), ИмяРеквизитаТЧ)));
		ИначеЕсли ЗначениеРеквизитаСтруктуры.Исключаемый Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Реквизит ""%1"" %2 исключаемого типа %3';
																						|en = 'Attribute ""%1"" of %2 is of excludable type %3'"), ИмяРеквизита,
				?(ПустаяСтрока(ИмяРеквизитаТЧ), "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'табличной части ""%1""';
																												|en = 'table ""%1""'"), ИмяРеквизитаТЧ)), ЗначениеРеквизитаСтруктуры.ТипЗначения);
		ИначеЕсли ЕстьДублиКолонок(ЭтотОбъект[ИмяТабличногоДокумента], ИмяРеквизита, ТекущаяОбласть.Лево) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Реквизит ""%1"" %2 уже есть в таблице.';
																						|en = 'Attribute ""%1"" %2 is already in the table.'"), ИмяРеквизита,
				?(ПустаяСтрока(ИмяРеквизитаТЧ), "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'табличной части ""%1""';
																												|en = 'table ""%1""'"), ИмяРеквизитаТЧ)));
		КонецЕсли;
...
Процедура РеквизитыПриИзменении(Элемент)
	...
	Элементы.РеквизитыПеречитать.Доступность = Истина;
...
Процедура СнятьФлажки(Команда)
	...
	Для Каждого Реквизит Из РеквизитыШапки Цикл
		...
		Если СтрСравнить(Реквизит.Имя, ИмяКлючевогоРеквизитаПоУмолчанию()) = 0 Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СкрытьПоказатьДеревоРеквизитов(Команда)
	...
	СкрытьПоказатьДеревоРеквизитовНаСервере();
...
Функция ЭтоПеречисление(ТипРеквизита)
	...
	Возврат ОбщегоНазначения.ЭтоПеречисление(МетаданныеРеквизита);
...
&НаСервереБезКонтекста
Функция ЭтоВидСчета(ТипРеквизита)
	...
	Если (ТипРеквизита = Тип("ВидСчета")) Или (ТипРеквизита = ОписаниеТиповВидСчета) Тогда
		Возврат Истина;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЭтоПланСчетов(ТипРеквизита)
	...
	Возврат ОбщегоНазначения.ЭтоПланСчетов(ЗначениеРеквизитаПоУмолчанию.Метаданные());
...
&НаСервереБезКонтекста
Функция ЭтоСтрока(ТипРеквизита)
	
	Если ТипРеквизита = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ОписаниеТиповБулево = Новый ОписаниеТипов("Строка");
	Для Каждого Тип Из ТипРеквизита.Типы() Цикл
...
&НаСервере
Функция СсылочноеЗначениеПоНаименованию(Наименование, ИмяРеквизита)
	
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1
		|	&ОбъектМетаданных КАК Ссылка
		|ИЗ
		|	&Таблица КАК ОбъектМетаданных
		|ГДЕ
		|	&Отбор = &Наименование";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Таблица", Объект.ОбъектМетаданных);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОбъектМетаданных", 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ОбъектМетаданных.%1", ИмяРеквизита));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Отбор", 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ОбъектМетаданных.%1.Наименование", ИмяРеквизита));
	...
	Иначе
		НайденноеЗначение = ПредопределенноеЗначение(Объект.ОбъектМетаданных + ".ПустаяСсылка");
		
		ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(Объект.ОбъектМетаданных);
		Если ОбъектМетаданных <> Неопределено Тогда
			РеквизитОбъектаМетаданных =  ОбъектМетаданных.Реквизиты.Найти(ИмяРеквизита);
			Если РеквизитОбъектаМетаданных <> Неопределено Тогда
				Для Каждого ТипыРеквизита Из РеквизитОбъектаМетаданных.Тип.Типы() Цикл
					Если ЭтоСсылка(ТипыРеквизита) Тогда
						ПолноеИмя = Метаданные.НайтиПоТипу(ТипыРеквизита).ПолноеИмя();
						НайденноеЗначение = ПредопределенноеЗначение(ПолноеИмя + ".ПустаяСсылка");
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ИменаКлючевыхРеквизитов = Новый СписокЗначений;
	ИменаКлючевыхРеквизитов.Добавить(ИмяКлючевогоРеквизитаПоУмолчанию());
	ИменаКлючевыхРеквизитов.Добавить(ИмяКлючевогоРеквизита);
	...
	ОтборЭлемента.ПравоеЗначение = ИменаКлючевыхРеквизитов;
...
Функция ВыделитьФрагментВСтроке(Знач Строка, Знач фрагмент)
	...
	ЖирныйШрифт = Новый Шрифт(,, Истина); // АПК:1345
...
Процедура СменитьСтраницуРаздела()
	...
	Если РежимФормы = "СтраницаТаблица" Тогда
		Элементы.ГруппаРазделы.ТекущаяСтраница = Элементы.СтраницаТаблицаСТЧ;
	ИначеЕсли РежимФормы = "СтраницаКод" Тогда
		Элементы.ГруппаРазделы.ТекущаяСтраница = Элементы.СтраницаКод;
	Иначе
		Элементы.ГруппаРазделы.ТекущаяСтраница = Элементы.СтраницаТаблица;
	КонецЕсли;
...
Процедура ТабличныйДокументВыборЗавершениеВыбораРеквизитаСсылочногоТипа(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ЭтоПеречисление(ДополнительныеПараметры.ТипЗначения)
		...
		И ЗначениеРеквизитаОбъекта(Результат, "Предопределенный")
		И ДополнительныеПараметры.ИмяРеквизита <> "Родитель" Тогда
...
// 
&НаКлиенте
Процедура ТабличныйДокументВыборЗавершениеВыбораРеквизитаВидаСчета(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Область.Текст = ПредставлениеВидаСчета(Результат.Значение);
	
	СтрокаДанных = ТаблицаДанных.Получить(ДополнительныеПараметры.НомерСтроки);
	СтрокаДанных.Вид = Результат.Значение;
...
Функция ТабличныйДокументПриИзмененииМодифицирован(Элемент)
	...
	Для НомерСтроки = ВерхОбласти По НизОбласти Цикл
		...
		Для НомерКолонки = ЛевоОбласти По ПравоОбласти Цикл
			...
			КоординатыОбласти.Вставить("Лево", НомерКолонки);
			ИмяОбласти = ИмяОбласти(КоординатыОбласти);
			Если Не СтруктураАктивнойОбласти.Свойство(ИмяОбласти, СтруктураЯчейки) Тогда
				Продолжить;
			КонецЕсли;
			Ячейка = ЭтотОбъект[ИмяТабличногоДокумента].Область(ИмяОбласти);
...
Процедура ТабличныйДокументСинхронизацияИзмененийДанных(Результат, ИмяРеквизита, ИмяТабличнойЧасти, НомерСтроки)
	...
	Если ПустаяСтрока(ИмяТабличнойЧасти) Тогда
		Если (ИмяРеквизита = ИмяКлючевогоРеквизита) И (ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизитаПоУмолчанию()) Тогда
			ЗаполнитьГУИДом = Ложь;
			СтрокаРезультат = Строка(Результат);
			Если (Не ПустаяСтрока(СтрокаРезультат)) И (СтрНайти(ИменаПредопределенныхЗначений, СтрокаРезультат + ",") = 0) Тогда
				// не существует предопределенного
				Если Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(СтрокаРезультат) Тогда
...
Процедура ТабличныйДокументПриИзмененииТипизацияЗначений(ИмяТабличнойЧасти)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДанныхОбласти Цикл
		...
		ИначеЕсли ЭтоВидСчета(ТипРеквизитаСтруктуры) Тогда
			СтрокаТаблицы.ТипОбласти = ТипРеквизитаСтруктуры;
			СтрокаТаблицы.Результат = ТекстОбласти;
...
Функция ПользовательскиеНастройкиЗаполненияЭлементов()
	...
	НастройкиЗаполненияЭлементов.Вставить("ИмяКлючевогоРеквизита",          ИмяКлючевогоРеквизитаПоУмолчанию());
...
&НаСервере
Функция ЗаполнитьТаблицуДанных(ПоКоду = Ложь)
	
	ЗаполнитьКолонкиТаблицыДанных();
	
	РезультатЗаполнения = РезультатВыполнения();
	
	Если ПоКоду Тогда
		РезультатЗаполнения = ЗаполнитьТаблицуДанныхПоКоду();
	Иначе
...
&НаСервере
Функция ЗаполнитьТаблицуДанныхПоКоду()
			...
			Иначе
				...
				Если Реквизит.Имя = ИмяКлючевогоРеквизита Тогда
					Если РезультатЗаполнения.СтруктураИмен.Свойство("Номер" + ТаблицаДанных.Количество(), ИмяПредопределенныхДанных) Тогда
						Если (ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизитаПоУмолчанию()) И ПустаяСтрока(ИмяПредопределенныхДанных) Тогда
							ЗначениеРеквизита = Строка(Новый УникальныйИдентификатор);
						Иначе
							ЗначениеРеквизита = ИмяПредопределенныхДанных;
						КонецЕсли;
					ИначеЕсли ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизитаПоУмолчанию() Тогда
						ЗначениеРеквизита = Строка(Новый УникальныйИдентификатор);
					КонецЕсли;
				КонецЕсли;
	...
	Если БезУдаленных И ТаблицаИсточник.Колонки.Найти(ИмяКлючевогоРеквизита) <> Неопределено Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаИсточник Цикл
			...
			Если Не СтрокаТаблицы.Предопределенный Тогда
				...
				КонецЕсли;
				Если ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизитаПоУмолчанию() Тогда
...
Функция ИнициализироватьТабличныеДокументы(ДанныеИнициализации = Неопределено)
	...
	ШрифтЗаголовка = Новый Шрифт(,, Истина); // АПК:1345
	...
	Для Каждого СтрокаДанных Из ИсточникДанных Цикл
		...
		Для Каждого Реквизит Из РеквизитыИнициализации Цикл
			...
			Если СодержитТолькоПростыеТипы(ТипЗначения) Тогда
				...
				Если (Реквизит.Имя = ИмяКлючевогоРеквизита) И (ИмяКлючевогоРеквизита <> ИмяКлючевогоРеквизитаПоУмолчанию())
					 И ПустаяСтрока(СтрокаДанных[ИмяКлючевогоРеквизита]) И ПустаяСтрока(СтрокаДанных[ИмяКлючевогоРеквизитаПоУмолчанию()]) Тогда
					Область.Примечание.Текст = НСтр("ru = 'Не заполнено значение ключевого реквизита.';
													|en = 'A key attribute is required.'");
					Область.ЦветФона = WebЦвета.Розовый;
				КонецЕсли;
			ИначеЕсли ЭтоВидСчета(ТипЗначения) Тогда
				Область.Текст = ПредставлениеВидаСчета(СтрокаДанных[Реквизит.Имя]);
				Область.Защита = Истина;
			ИначеЕсли ЭтоПеречисление(ТипЗначения) Тогда
				Область.Текст = Строка(СтрокаДанных[Реквизит.Имя]);
			ИначеЕсли (ЭтоСсылочныйТип(ТипЗначения) И ЗначениеЗаполнено(СтрокаДанных[Реквизит.Имя]))
				  Или ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СтрокаДанных[Реквизит.Имя])) Тогда
				Область.СодержитЗначение = Ложь;
				Область.Текст = Строка(СтрокаДанных[Реквизит.Имя]);
			...
			Область.Защита = (Область.Защита Или (Реквизит.Имя = ИмяКлючевогоРеквизита));
...
Процедура ЗаполнитьДинамическиеЭлементыФормы()
	...
	Для Каждого Реквизит Из ПолнаяСтруктураОбъекта Цикл
		...
		Если (СтрСравнить(ИмяТабличнойЧасти, "Представления") = 0)
			Или (СтрСравнить(ИмяТабличнойЧасти, "КонтактнаяИнформация") = 0)
			Или (СтрСравнить(ИмяТабличнойЧасти, "ДополнительныеРеквизиты") = 0) Тогда
...
&НаКлиенте
Процедура ОбновитьТаблицуИКод()
	
	ТекущаяСтраница = Элементы.ГруппаРазделы.ТекущаяСтраница;
	Если ТекущаяСтраница = Элементы.СтраницаКод Тогда
		
		ПреобразоватьВТаблицуНаСервере(Ложь);
		ПреобразоватьВКодНаСервере();
		ТекущиеИмяПредопределенныхДанных = "";
		ИмяОбластиШапки = "";
	...
	Элементы.РеквизитыПеречитать.Доступность = Ложь;
...
Процедура ПреобразоватьТабличныеДанныеВКод(ИсточникДанных, ДанныеИнициализации = Неопределено, ИмяТабличнойЧасти = "")
	...
	Для Каждого СтрокаДанных Из ИсточникДанных Цикл
		...
		ЗначениеКлючевогоРеквизитаЗаполнено = Истина;
		
		Если ЕстьКлючеваяКолонка И ПустаяСтрока(ИмяТабличнойЧасти) И ВыводитьИменПредопределенныхДанных Тогда
			ЗначениеКлючевогоРеквизита = СтрокаДанных[ИмяКлючевогоРеквизита];
			ЗначениеИмениПредопределенныхДанных = СтрокаДанных.ИмяПредопределенныхДанных;
			
			Если ПустаяСтрока(ЗначениеКлючевогоРеквизита) И ПустаяСтрока(ЗначениеИмениПредопределенныхДанных) Тогда
				ЗначениеКлючевогоРеквизитаЗаполнено = Ложь;
			КонецЕсли;
			
			Если ПустаяСтрока(ЗначениеКлючевогоРеквизита) Тогда
				ЗначениеКлючевогоРеквизита = ЗначениеИмениПредопределенныхДанных;
			КонецЕсли;
			
			Если ЗначениеКлючевогоРеквизитаЗаполнено И (Не ЗначениеЗаполнено(ЗначениеКлючевогоРеквизита))
				Или (УникальныеИменаСтрок.Получить(ЗначениеКлючевогоРеквизита) = Истина) Тогда
				// строка с таким ИменемПредопределенныхДанных уже была
				Продолжить;
			КонецЕсли;
			УникальныеИменаСтрок.Вставить(ЗначениеКлючевогоРеквизита, Истина);
		...
		Для Каждого Реквизит Из РеквизитыИнициализации Цикл
			...
			ЭтоВидСчета   = ЭтоВидСчета(ТипРеквизита);
			...
			ИначеЕсли ЭтоПростойТип И ОписаниеТиповСтрока.СодержитТип(ТипРеквизита) И (Не Локализуемый) Тогда
				
				Если НЕ ЗначениеЗаполнено(ЗначениеРеквизита) И ЗначениеКлючевогоРеквизитаЗаполнено Тогда
					Продолжить;
				КонецЕсли;
				...
				Если ИмяРеквизита <> ИмяКлючевогоРеквизита Тогда
					...
					КонецЕсли;
				ИначеЕсли (СтрНайти(ИменаПредопределенныхЗначений, ЗначениеРеквизита + ",") > 0)
					 Или (ИмяКлючевогоРеквизита <> ИмяКлючевогоРеквизитаПоУмолчанию()) Тогда
				...
				КомментарийДляКода = "";
				Если (ИмяРеквизита = ИмяКлючевогоРеквизита) И ПустаяСтрока(ЗначениеРеквизита) Тогда
					// Исключение из проверки на недопустимые комментарии в коде 
					КомментарийДляКода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '// %1 Заполнить уникальным значением';
																										|en = '// %1 Replace with a unique value'"), "TO" + "DO");
				КонецЕсли;
				
				Код1С.ДобавитьСтроку(Символы.Таб + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;%4",
					Элемент, ИмяРеквизита, ЗначениеРеквизитаСтрока, КомментарийДляКода));
			...
			ИначеЕсли ЭтоВидСчета Тогда
				ИмяВидаСчета = ИмяВидаСчетаПоЗначению(ЗначениеРеквизита);
				Код1С.ДобавитьСтроку(Символы.Таб + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;", Элемент, ИмяРеквизита, "ВидСчета." + ИмяВидаСчета));
			Иначе // Если ЭтоСсылочныйРеквизит Тогда
				...
				ИначеЕсли Метаданные.Справочники.Найти(МетаданныеРеквизита.Имя) <> Неопределено
					...
					Предопределенный = (СтрСравнить(ИмяКлючевогоРеквизита, ИмяКлючевогоРеквизитаПоУмолчанию()) = 0 И ЗначениеРеквизита.Предопределенный)
...
&НаСервере
Функция ПреобразоватьВТаблицуНаСервере(ПоКоду = Истина)
	
	РезультатПреобразования = РезультатВыполнения();
	
	РезультатЗаполнения = ЗаполнитьТаблицуДанных(ПоКоду);
	Если Не РезультатЗаполнения.Успешно Тогда
		ЗаполнитьЗначенияСвойств(РезультатПреобразования, РезультатЗаполнения);
		Возврат РезультатПреобразования;
	КонецЕсли;
	
	ЗаполнитьТабличныеДокументы(ПоКоду);
...
Функция ПолучитьРезультатЗаполненияПоКоду()
	...
	РезультатЗаполнения = РезультатВыполненияКодаСДанными();
	...
	ТекстКода1С = СтрЗаменить(ТекстКода1С, "МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит", "МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизитНачальногоЗаполнения");
	...
	Попытка
		ОбщегоНазначения.ВыполнитьВБезопасномРежиме(ТекстКода1С, ПараметрыВыполнения);
	Исключение
		
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Причина = ?(ТипЗнч(ИнформацияОбОшибке.Причина) = Тип("ИнформацияОбОшибке"), ИнформацияОбОшибке.Причина, ИнформацияОбОшибке);
		СтрокаСОписаниемОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(Причина);
		
		РезультатЗаполнения.ТекстОшибки = СтрокаСОписаниемОшибки;
		РезультатЗаполнения.Успешно     = Ложь;
		
		ИскомаяСтрока = "{(";
		ПозицияСтрокиОшибки = СтрНайти(СтрокаСОписаниемОшибки, ИскомаяСтрока);
		
		Если ПозицияСтрокиОшибки = 0 Тогда
			Возврат РезультатЗаполнения;
		КонецЕсли;
		
		ПозицияОткрывающейСкобки = ПозицияСтрокиОшибки + СтрДлина(ИскомаяСтрока);
		ПозицияЗакрывающейСкобки = СтрНайти(СтрокаСОписаниемОшибки, ")",, ПозицияСтрокиОшибки);
		КоличествоСимволовНомераСтроки = ПозицияЗакрывающейСкобки - ПозицияОткрывающейСкобки;
		
		ФрагментСНомеромСтрокиОшибки = Сред(СтрокаСОписаниемОшибки, ПозицияОткрывающейСкобки, КоличествоСимволовНомераСтроки);
		ФрагментСНомеромСтрокиОшибки = СтрЗаменить(ФрагментСНомеромСтрокиОшибки, " ", "");
		
		Попытка
			НомерСтрокиОшибки = Число(ФрагментСНомеромСтрокиОшибки);
		Исключение
			Возврат РезультатЗаполнения;
		КонецПопытки;
		...
		СтрокаСОшибкой = СтрПолучитьСтроку(ТекстКода1С, НомерСтрокиОшибки);
		НачальнаяПозиция = СтрНайти(ТекстКода1С, СтрокаСОшибкой);
		Если НачальнаяПозиция = 0 Тогда
			Возврат РезультатЗаполнения;
		КонецЕсли;
		
		КонечнаяПозиция = НачальнаяПозиция + СтрДлина(СтрокаСОшибкой);
		Элементы.Код1С.УстановитьГраницыВыделения(НачальнаяПозиция, КонечнаяПозиция);
		
		ТекущийЭлемент = Элементы.Код1С;
		РежимФормы = "СтраницаКод";
	...
	РезультатЗаполнения.Данные        = ПараметрыВыполнения.МассивОбъектов;
	РезультатЗаполнения.СтруктураИмен = ПараметрыВыполнения.СтруктураИмен;
...
&НаСервере
Функция КодДляВыполнения()
	
	КлючевоеСловоПроцедура      = "Процедура";
	КлючевоеСловоКонецПроцедуры = "КонецПроцедуры";
	
	ТекстКода1С = СокрЛП(Код1С.ПолучитьТекст());
	ТекстКода1С = СтрЗаменить(ТекстКода1С,  КлючевоеСловоПроцедура, "//" + КлючевоеСловоПроцедура);
	ТекстКода1С = СтрЗаменить(ТекстКода1С, КлючевоеСловоКонецПроцедуры, "//" + КлючевоеСловоКонецПроцедуры);
...
&НаСервере
Процедура ЗаписатьКопиюТабличногоДокумента(Доступность = Истина)
	
	КопияТабличногоДокумента.Очистить();
	КопияТабличногоДокумента.Вывести(ТабличныйДокумент);
	
	ТаблицаДанныхНаСервере = РеквизитФормыВЗначение("ТаблицаДанных", Тип("ТаблицаЗначений"));
	АдресХранилищаТаблицыДанных = ПоместитьВоВременноеХранилище(ТаблицаДанныхНаСервере, Новый УникальныйИдентификатор);
	
	Элементы.ВернутьсяКПоследнейРабочейВерсии.Доступность = Доступность;
	
КонецПроцедуры

&НаСервере
Процедура ВосстановитьТабличныйДокументИзКопии()
	
	ТабличныйДокумент.Очистить();
	ТабличныйДокумент.Вывести(КопияТабличногоДокумента);
	
	ТаблицаДанныхНаСервере = ПолучитьИзВременногоХранилища(АдресХранилищаТаблицыДанных);
	ЗначениеВРеквизитФормы(ТаблицаДанныхНаСервере, "ТаблицаДанных");
	
	Элементы.ВернутьсяКПоследнейРабочейВерсии.Доступность = Ложь;
	
КонецПроцедуры

&НаКлиенте
Функция РезультатПроверкиДанныхЯчейки(ТекущаяОбласть, ТипРеквизита, ИмяРеквизита)
	
	СтруктураРезультат = РезультатПроверки();
	
	Попытка
		ЗначениеЯчейки = ТекущаяОбласть.Значение;
		СтруктураРезультат.ЭтоЗначение = Истина;
	Исключение
		ЗначениеЯчейки = ТекущаяОбласть.Текст;
	КонецПопытки;
	
	Если ТипРеквизита = Неопределено Тогда
		ШаблонТекстаОшибки = НСтр("ru = 'Не удалось определить описание реквизита ""%1""';
									|en = 'Не удалось определить описание реквизита ""%1""'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстаОшибки, ИмяРеквизита);
		
		СтруктураРезультат.Успешно     = Ложь;
		СтруктураРезультат.ТекстОшибки = ТекстОшибки;
		СтруктураРезультат.Значение    = ЗначениеЯчейки;
		
		Возврат СтруктураРезультат;
	КонецЕсли;
	
	Если ЭтоВидСчета(ТипРеквизита) И (ТипЗнч(ЗначениеЯчейки) <> Тип("ВидСчета")) Тогда
		СтруктураРезультат.Успешно = Ложь;
		СтруктураРезультат.ТекстОшибки = НСтр("ru = 'Некорректное значение вида счета.';
												|en = 'Invalid account type.'");
		СтруктураРезультат.Значение = СтруктураАктивнойОбласти[ТекущаяОбласть.Имя].Значение;
		Возврат СтруктураРезультат;
	КонецЕсли;
	
	Если ЭтоСсылочныйТип(ТипРеквизита) Тогда
		ЗначениеПоНаименованию = СсылочноеЗначениеПоНаименованию(ЗначениеЯчейки, ИмяРеквизита);
		Если (Не ЗначениеПоНаименованию.Пустая()) Или ПустаяСтрока(ЗначениеЯчейки) Тогда
			СтруктураРезультат.Значение = ЗначениеПоНаименованию;
			Возврат СтруктураРезультат;
		КонецЕсли;
		
		СтруктураРезультат.Успешно = Ложь;
		СтруктураРезультат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Некорректные данные для типа ""%1"".';
																										|en = 'Invalid value for the %1 data type.'"),
			ТипРеквизита);
		Возврат СтруктураРезультат;
	КонецЕсли;
	
	Если (ИмяРеквизита = ИмяКлючевогоРеквизитаПоУмолчанию()) Или (ИмяРеквизита = ИмяКлючевогоРеквизита) Тогда
		Индекс = ТекущаяОбласть.Низ - 2;
		Попытка
			СтрокаДанных = ТаблицаДанных.Получить(Индекс);
		Исключение
			Возврат СтруктураРезультат;
		КонецПопытки;
		
		Если ПустаяСтрока(ЗначениеЯчейки) И ((ТекущаяОбласть.Лево = 1) Или (ИмяРеквизита = ИмяКлючевогоРеквизита)) Тогда
			СтруктураРезультат.Успешно = Ложь;
			СтруктураРезультат.ТекстОшибки = НСтр("ru = 'Значение ключевого реквизита не может быть пустым.';
													|en = 'Key attributes require a non-empty value.'");
			СтруктураРезультат.Значение = СтруктураАктивнойОбласти[ТекущаяОбласть.Имя].Значение;
			Возврат СтруктураРезультат;
		КонецЕсли;
		
		НайденныеСтроки = ТаблицаДанных.НайтиСтроки(Новый Структура(ИмяРеквизита, ЗначениеЯчейки));
		Если НайденныеСтроки.Количество() > 0 Тогда
			СтруктураРезультат.Успешно = Ложь;
			СтруктураРезультат.ТекстОшибки = НСтр("ru = 'Значение ключевого реквизита не уникально.';
		...
		Если ИмяКлючевогоРеквизита <> ИмяКлючевогоРеквизитаПоУмолчанию() Тогда
			Если СуществуетПредопределенный(ЗначениеЯчейки) Тогда
			...
			ИмяПроверяемогоРеквизита = ?(ИмяРеквизита = ИмяКлючевогоРеквизита, ИмяКлючевогоРеквизитаПоУмолчанию(), ИмяКлючевогоРеквизита);
			Если Не ПустаяСтрока(СтрокаДанных[ИмяПроверяемогоРеквизита]) Тогда
				ТекущаяОбласть.Значение = "";
				Возврат СтруктураРезультат;
			КонецЕсли;
		КонецЕсли;
	...
	Если ЭтоСтрока(ТипРеквизита) Тогда
		Возврат СтруктураРезультат;
	КонецЕсли;
	
	Если ЭтоЧисло(ТипРеквизита) Тогда
		Если ТипЗнч(ЗначениеЯчейки) = Тип("Строка") Тогда
			СтрокаБезПробелов = СтрЗаменить(ЗначениеЯчейки, " ", "");
			Попытка
				Число = Число(СтрокаБезПробелов);
			Исключение
				СтруктураРезультат.Успешно = Ложь;
				СтруктураРезультат.ТекстОшибки = НСтр("ru = 'Некорректные данные для типа ""Число"".';
	...
	Если ЭтоБулево(ТипРеквизита) Тогда
		Если ТипЗнч(ЗначениеЯчейки) = Тип("Строка") Тогда
			СтрокаБезПробелов = СтрЗаменить(ЗначениеЯчейки, " ", "");
			Попытка
				Булево = Булево(СтрокаБезПробелов);
			Исключение
				СтруктураРезультат.Успешно = Ложь;
				СтруктураРезультат.ТекстОшибки = НСтр("ru = 'Некорректные данные для типа ""Булево"".';
	...
	Если ЭтоПеречисление(ТипРеквизита) Тогда
		ЗначениеСуществует = ЗначениеПеречисленияСуществует(ЗначениеЯчейки, ТипРеквизита);
		Если Не ЗначениеСуществует Тогда
			СтруктураРезультат.Успешно = Ложь;
			СтруктураРезультат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Некорректные данные для типа ""%1"".';
																											|en = 'Invalid value for the %1 data type.'"),
				ТипРеквизита);
...
&НаСервере
Функция СуществуетПредопределенный(ИмяПредопределенныхДанных)
	
	ТекстЗапроса = ПолучитьТекстЗапросаВыборкиДанных();
	Запрос = Новый Запрос(ТекстЗапроса);
	ТаблицаИсточник = Запрос.Выполнить().Выгрузить();
	
	НайденныеСтроки = ТаблицаИсточник.НайтиСтроки(Новый Структура("ИмяПредопределенныхДанных", ИмяПредопределенныхДанных));
...
&НаСервере
Функция ЗначениеПеречисленияСуществует(Значение, ТипРеквизита)
	...
	Для Каждого Тип Из ТипРеквизита.Типы() Цикл
		...
		Возврат (ОбъектМетаданныхПоТипу.ЗначенияПеречисления.Найти(Значение) <> Неопределено);
	КонецЦикла;
...
&НаСервере
Процедура СкрытьПоказатьДеревоРеквизитовНаСервере()
	
	ВидимостьДерева = Не Элементы.ГруппаСвойства.Видимость;
	
	Элементы.СкрытьДеревоРеквизитов.Пометка = ВидимостьДерева;
	Элементы.ГруппаСвойства.Видимость       = ВидимостьДерева;
	
	КомандаФормы = Команды.Найти("СкрытьПоказатьДеревоРеквизитов");
	КомандаФормы.Подсказка = ?(ВидимостьДерева,
		НСтр("ru = 'Скрыть дерево реквизитов';
			|en = 'Hide attribute tree'"),
		НСтр("ru = 'Показать дерево реквизитов';
			|en = 'Show attribute tree'"));
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ИмяВидаСчетаПоЗначению(Значение)
	
	Если Значение = ВидСчета.АктивноПассивный Тогда
		Возврат "АктивноПассивный";
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПредставлениеВидаСчета(Значение)
	
	ВидСчетаСтрока = "";
	
	Если Значение = ВидСчета.АктивноПассивный Тогда
		ВидСчетаСтрока = "АП";
	ИначеЕсли Значение = ВидСчета.Активный Тогда
		ВидСчетаСтрока = "А";
	ИначеЕсли Значение = ВидСчета.Пассивный Тогда
		ВидСчетаСтрока = "П";
	КонецЕсли;
	
	Возврат ВидСчетаСтрока;
...
&НаСервере
Функция РезультатВыполненияКодаСДанными()
	
	Результат = РезультатВыполнения();
	Результат.Вставить("Данные",        Новый Массив);
	Результат.Вставить("СтруктураИмен", Новый Структура);
	Возврат Результат;
...
Функция РезультатПроверки()
	...
	Результат.Вставить("ЭтоЗначение", Ложь);
	Возврат Результат;
