...
Процедура ПеренестиФайлы(Команда)
	...
	Если ПустаяСтрока(ДокументИлиСправочник) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите документ или справочник с присоединенными файлами.';
														|en = 'Specify a document or catalog with attachments.'"),, "ДокументИлиСправочник");
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(Источник) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите таблицу-источник с файлами.';
														|en = 'Specify a source table with files.'"),, "Источник");
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(Приемник) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите таблицу-приемник файлов.';
														|en = 'Specify a file destination table.'"),, "Приемник");
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = Элементы.ВладельцыПрисоединенныхФайлов.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выберите владельца присоединенных файлов.';
														|en = 'Select the owner of attachments.'"),, "ВладельцыПрисоединенныхФайлов");
		Возврат;
	КонецЕсли;
	...
	Если Результат > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файлы успешно перенесены (%1).';
				|en = 'Files are transferred successfully (%1).'"), Результат);
	Иначе
...
Процедура СоздатьФайлыДляПереноса(Команда)
	...
	Если ПустаяСтрока(ДокументИлиСправочник) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите объект метаданных с файлами.';
														|en = 'Specify a metadata object with files.'"),, "ДокументИлиСправочник");
		Возврат;
	КонецЕсли;
	...
	Если Результат > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файлы успешно созданы (%1).';
				|en = 'Files are created successfully (%1).'"), Результат);
	Иначе
