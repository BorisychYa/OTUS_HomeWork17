...
Функция ПолучитьДеталиОшибки(ДанныеОбОшибке)
	...
	Если СтрНайти(ТекстИсключения, "InvalidCertificateIdError") Тогда
		...
		Если Позиция > 0 Тогда
			Результат.Вставить("ОписаниеОшибки", НСтр("ru = 'Не обнаружен сертификат в приложении';
														|en = 'The certificate is not found in the app'") + ": " + Сред(ТекстИсключения, Позиция + 23, 40) + ".");
		Иначе	
			Результат.Вставить("ОписаниеОшибки", НСтр("ru = 'Не обнаружен сертификат в приложении.';
														|en = 'The certificate is not found in the app.'"));
		КонецЕсли;	
		Результат.Вставить("Информация", НСтр("ru = 'Указанный сертификат не обнаружен в приложении.';
												|en = 'The certificate is not found in the app.'"));
	КонецЕсли;
