...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ИспользоватьСервисСклоненияMorpher.РасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
		...
		|en = 'Allow usage of Morpher declension service to get an alternative object presentation declension in grammar cases (Internet connection is required).
		|If the external service is not used, presentation declension is carried out using the built-in app tools.'"));
	// Конец Локализация
...
Процедура СписокНовостей_ИнтервалАвтообновленияПриИзменении(Элемент)
	...
	// Достаточно просто сбросить кэш, чтобы значения перезаполнились.
	ОбновитьПовторноИспользуемыеЗначения();
