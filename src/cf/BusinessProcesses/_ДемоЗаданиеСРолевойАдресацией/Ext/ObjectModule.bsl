...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Пользователи.ЭтоСеансВнешнегоПользователя() И НедопустимыйИсполнительДляВнешнегоПользователя() Тогда
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПоручениеИсполнителюПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Задача, "ДатаИсполнения") > ТекущаяДатаСеанса() Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Фактическая дата выполнения задачи не может быть больше текущей даты.';
				|en = 'The date when the duty is completed cannot be later than today.'"), Задача,
...
Процедура ПроверитьПриСозданииВложенныхБизнесПроцессов(ТочкаМаршрутаБизнесПроцесса, ФормируемыеБизнесПроцессы, Отказ)
	...
	Для Каждого БизнесПроцессПроверки Из ФормируемыеБизнесПроцессы Цикл
		...
		БизнесПроцессПроверки.Наименование = НСтр("ru = 'Проверить:';
													|en = 'Revise:'") + " " + Наименование;
...
Процедура ОбработкаЗаполненияДляВнешнегоПользователя(Знач СчетНаОплатуПокупателю)
	...
	Запрос.УстановитьПараметр("ОбъектАвторизации", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетНаОплатуПокупателю, "Партнер"));
...
Функция НедопустимыйИсполнительДляВнешнегоПользователя()
	...
	Если ТипЗнч(РольИсполнителя) = ТипЗнч(Справочники.РолиИсполнителей.ПустаяСсылка()) 
