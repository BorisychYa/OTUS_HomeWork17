...
Процедура ПодготовитьИПрикрепитьОбластьДанных(Знач КодОбластиДанных, Знач СписокПользователей, Знач ПредставлениеПриложения, Знач ЧасовойПоясПриложения, Знач ИдентификаторФайлаНачальныхДанных) Экспорт
	...
	Исключение
		...
		УведомитьМенеджерОшибкаПодготовкиОбласти(КодОбластиДанных, 
...
Процедура ПодготовитьОбластьИзВыгрузки(Знач КодОбластиДанных, Знач ИдентификаторВыгрузки, 
		Знач СопоставлениеПользователей, Знач ОбщиеДанные = Ложь, ИдентификаторСостояния = Неопределено) Экспорт
	...
	Попытка
		...
		ОтборЗадания.Вставить("ИмяМетода", РаботаВМоделиСервиса.ИмяМетодаОчиститьОбластьДанных());
		...
		Если Не ДанныеЗаблокированы
			...
			ПараметрыПодготовкиОбласти = ПараметрыПодготовкиОбласти(
				КодОбластиДанных,
		...
		Если ЗначениеЗаполнено(ИдентификаторСостояния) Тогда
...
Функция ПараметрыПодготовкиОбласти(КодОбластиДанных, ИдентификаторВыгрузки, СопоставлениеПользователей, ОбщиеДанные, ИдентификаторСостояния) Экспорт
	ПараметрыПодготовки = Новый Массив;
	ПараметрыПодготовки.Добавить(КодОбластиДанных);
	ПараметрыПодготовки.Добавить(ИдентификаторВыгрузки);
	ПараметрыПодготовки.Добавить(СопоставлениеПользователей);
	ПараметрыПодготовки.Добавить(ОбщиеДанные);
	ПараметрыПодготовки.Добавить(ИдентификаторСостояния);
	Возврат ПараметрыПодготовки;
КонецФункции

Процедура ДобавитьЗаданиеПодготовитьОбластьИзВыгрузки(ПараметрыПодготовкиОбласти, КодОбластиДанных, Неразделенное, Отложенное = Ложь) Экспорт 

	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("ИмяМетода", РаботаВМоделиСервиса.ИмяМетодаПодготовитьОбластьИзВыгрузки());
	ПараметрыЗадания.Вставить("Параметры", ПараметрыПодготовкиОбласти);
	
	Если Неразделенное Тогда
	...
	Если Отложенное Тогда
		СекундВ10Минутах = 600;
		ПараметрыЗадания.Вставить("ЗапланированныйМоментЗапуска", ТекущаяДатаСеанса() + СекундВ10Минутах);
	КонецЕсли;
	
	ПараметрыЗадания.Вставить("Ключ", XMLСтрока(КодОбластиДанных));	
...
Процедура ПриОшибкеПодготовкиОбластиИзВыгрузки(ПараметрыЗадания, ИнформацияОбОшибке) Экспорт
	...
	ПараметрыПодготовки = ПараметрыЗадания.Параметры;
	ОбластьДанных = ПараметрыПодготовки[0];
	
	МенеджерОбластиДанных = РегистрыСведений.ОбластиДанных.СоздатьМенеджерЗаписи();
	МенеджерОбластиДанных.ОбластьДанныхВспомогательныеДанные = ОбластьДанных;
	МенеджерОбластиДанных.Прочитать();
	
	Если Не МенеджерОбластиДанных.Выбран() Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Область данных %1 не найдена';
										|en = '%1 data area is not found'"), ОбластьДанных);
	КонецЕсли;
			
	ОтборЗаданий = Новый Структура();
	
	КлючЗадания = XMLСтрока(ОбластьДанных);
	ОтборЗаданий.Вставить(
		"Ключ",
		КлючЗадания); 
	ОтборЗаданий.Вставить(
		"ИмяМетода",
		РаботаВМоделиСервиса.ИмяМетодаПодготовитьОбластьИзВыгрузки());

	Задания = ОчередьЗаданий.ПолучитьЗадания(ОтборЗаданий);
	
	Если Не ЗначениеЗаполнено(Задания) Тогда		
		ОтборЗаданий.Удалить("Ключ");
		ВсеЗадания = ОчередьЗаданий.ПолучитьЗадания(ОтборЗаданий);
		Для Каждого Задание Из ВсеЗадания Цикл
			СтрокиПоОбласти = Новый Массив;
			Если Задание.Параметры[0] = ОбластьДанных Тогда
				СтрокиПоОбласти.Добавить(Задание);	
			КонецЕсли;			
		КонецЦикла;
		Задания = ВсеЗадания.Скопировать(СтрокиПоОбласти);
	КонецЕсли;
	
	АварийноеЗадание = Задания.Найти(
		Перечисления.СостоянияЗаданий.ОбработкаОшибкиПриАварийномЗавершении,
		"СостояниеЗадания");
		
	ЧастиКомментария = Новый Массив;

	Если АварийноеЗадание = Неопределено Тогда
		ЧастиКомментария.Добавить(НСтр("ru = 'Процесс подготовки области данных завершился с ошибкой';
										|en = 'Data area preparation completed with error'"));		
	Иначе	
		ЧастиКомментария.Добавить(НСтр("ru = 'Процесс подготовки области данных аварийно завершился';
										|en = 'Data area preparation crashed'"));		
	КонецЕсли;
	
	Повтор = МенеджерОбластиДанных.Повтор;
	
	ПовторПриАварийномЗавершении = МенеджерОбластиДанных.ПовторПриАварийномЗавершении;
	МаксимальноеКоличествоПовторовПриАварийномЗавершении = 15;	
	
	ПовторПриЗавершенииПоОшибке = Макс(Повтор - ПовторПриАварийномЗавершении, 0);
	МаксимальноеКоличествоПовторовПриЗавершенииПоОшибке = 3;
	
	ПревышеноКоличествоПопыток = ПовторПриЗавершенииПоОшибке > МаксимальноеКоличествоПовторовПриЗавершенииПоОшибке 
		Или ПовторПриАварийномЗавершении > МаксимальноеКоличествоПовторовПриАварийномЗавершении;

	Если ПревышеноКоличествоПопыток Тогда
		ЧастиКомментария.Добавить(НСтр("ru = 'Исчерпано количество попыток подготовки.';
	...
	Иначе
		...
		Если АварийноеЗадание = Неопределено Тогда
			ПовторПриЗавершенииПоОшибке = ПовторПриЗавершенииПоОшибке + 1;	
		Иначе
			ПовторПриАварийномЗавершении = ПовторПриАварийномЗавершении + 1;		
		КонецЕсли;
		
		ЧастиКомментария.Добавить(НСтр("ru = 'Запланирована следующая попытка подготовки.';
										|en = 'The next preparation attempt is scheduled.'"));
		
	КонецЕсли;
		
	ОписаниеПопыток = СтрШаблон(НСтр("ru = 'Повтор при ошибке %1 из %2
		|Повтор при аварийном завершении %3 из %4';
		|en = 'Retry on error %1 out of %2
		|Retry on abnormal termination %3 out of %4'"),
		ПовторПриЗавершенииПоОшибке,
		МаксимальноеКоличествоПовторовПриЗавершенииПоОшибке,
		ПовторПриАварийномЗавершении,
		МаксимальноеКоличествоПовторовПриАварийномЗавершении);
	ЧастиКомментария.Добавить(ОписаниеПопыток);	
	
	ВходитьВОбласть = Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных();
	
	НачатьТранзакцию();	
	Попытка
				
		МенеджерОбластиДанных.Повтор = Повтор;
		МенеджерОбластиДанных.ПовторПриАварийномЗавершении = ПовторПриАварийномЗавершении;
		МенеджерОбластиДанных.Записать();		
		...
		Иначе
			...
			ОбщиеДанные = ПараметрыПодготовки[3];
			
			ПараметрыПодготовкиОбласти = ПараметрыПодготовкиОбласти(
				ОбластьДанных,
				ПараметрыПодготовки[1],
				ПараметрыПодготовки[2],
				ОбщиеДанные,
				ПараметрыПодготовки[4]);
		...
		Если ВходитьВОбласть Тогда
			РаботаВМоделиСервиса.ВойтиВОбластьДанных(ОбластьДанных);	
		КонецЕсли;
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Область данных.Ошибка при загрузке из файла';
				|en = 'Data area.Error importing from file'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Предупреждение,,,
			СтрСоединить(ЧастиКомментария, Символы.ПС),
...
Процедура ПрикрепитьОбластьДанных(Параметры) Экспорт
	...
	Попытка
		...
		ОбновитьКодПредопределенныхУзлов();
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(РаботаВМоделиСервиса.ИмяМетодаПодготовитьОбластьИзВыгрузки());
...
Процедура ПриОпределенииОбработчиковОшибок(ОбработчикиОшибок) Экспорт
	...
	ОбработчикиОшибок.Вставить(РаботаВМоделиСервиса.ИмяМетодаПодготовитьОбластьИзВыгрузки(), "РаботаВМоделиСервисаБТС.ПриОшибкеПодготовкиОбластиИзВыгрузки");
