...
Функция ПровестиСчетаФактурыВФоне()
	...
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Проведение счет-фактур контрагента ""%1""';
				|en = 'Post tax invoices of counterparty %1'"), Контрагент);
	КонецЕсли;
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Документы._ДемоСчетФактураПолученный.ПровестиСчетаФактурыПоПроблемнымКонтрагентам",
...
Функция ОписаниеПричинНеуспеха(АдресРезультата, КоличествоПроведенных, КоличествоНепроведенных, КоличествоНеправильноЗаполненных)
	...
	ГиперссылкаОтчета = Новый ФорматированнаяСтрока(НСтр("ru = 'Подробнее...';
														|en = 'Details…'"), , , , АдресРезультата);
...
Процедура НадписьНеуспешноеИсправлениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СписокПроблемныхСчетовФактур.Показать(НСтр("ru = 'Список проблемных счетов-фактур';
												|en = 'List of tax invoices with issues'"), , Истина);
