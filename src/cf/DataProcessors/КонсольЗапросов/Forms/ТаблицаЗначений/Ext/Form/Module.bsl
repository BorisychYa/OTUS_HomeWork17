...
Процедура НастройкиТаблицыЗначенийНаименованиеКолонкиОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПоказатьСообщениеПользователю(НСтр("ru = 'Наименование колонки не может быть пустым.';
											|en = 'Column title cannot be blank.'"), "Объект");
	КонецЕсли;
...
Процедура ЗаполнитьТаблицыПриОткрытии()
	...
	Для каждого СтрокаТаблицы Из ТЗ Цикл
		...
		Для каждого Колонка Из ТЗ.Колонки Цикл
			ЭлементТЗ[Колонка.Имя]  = СтрокаТаблицы[Колонка.Имя];
		КонецЦикла;
	КонецЦикла;	
КонецПроцедуры	
