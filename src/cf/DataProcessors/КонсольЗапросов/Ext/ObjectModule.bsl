...
Функция ПолучитьИмяВременнойТаблицы(ТекстЗапроса, Буфер, Позиция)
	...
	Возврат НСтр("ru = 'Временная таблица';
				|en = 'Temporary table'") + ": " + ИмяТаблицы;
...
Функция ФормированиеПредставленияЗначения(Значение) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("ТаблицаЗначений") Тогда
		ИтоговаяСтрока = НСтр("ru = 'Таблица: строк = %КоличествоСтрок%, колонок = %КоличествоКолонок%';
								|en = 'Table: rows = %КоличествоСтрок%, columns = %КоличествоКолонок%'");
...
Функция БлокКонфигурацииСеанса(ФильтрТекущегоСеанса, ШаблоныТегов)
	...
	СобытияТехнологическогоЖурнала = СобытияКонсолиЗапросов(ИнформационнаяБазаФайловая);
	КаталогСЛогФайлами = ПолучитьИмяВременногоФайла("1c_logs");
	
	ШаблонФильтраSDBL = "  <eq property=""DBMS"" value=""DBV8DBEng""/>
		|  <eq property=""DataBase"" value=""%1""/>";
	ШаблонФильтраEXCP = "  <eq property=""DataBase"" value=""%1""/>";
	ШаблонФильтра = "  <eq property=""p:processName"" value=""%1""/>
		|  <eq property=""usr"" value=""%2""/>
		|  <eq property=""sessionid"" value=""%3""/>";

	Текст = Новый Массив;
	Текст.Добавить(ТэгТекущегоСеанса(ШаблоныТегов, ФильтрТекущегоСеанса));
	Текст.Добавить(СтрЗаменить("<log history=""2"" location=""%1"">", "%1", КаталогСЛогФайлами));
	Для каждого ЗначениеСобытия Из СобытияТехнологическогоЖурнала Цикл
		
		Если ИнформационнаяБазаФайловая Тогда
			Если ЗначениеСобытия = "SDBL" Тогда
				ТекстФильтров = СтрЗаменить(ШаблонФильтраSDBL, "%1", ФильтрТекущегоСеанса.DataBase);
			ИначеЕсли ЗначениеСобытия = "DBV8DBEng" Или ЗначениеСобытия = "EXCP" Тогда
				ТекстФильтров = СтрЗаменить(ШаблонФильтраEXCP, "%1", ФильтрТекущегоСеанса.DataBase);
			КонецЕсли;
		Иначе
			ТекстФильтров = СтрЗаменить(ШаблонФильтра, "%1", ФильтрТекущегоСеанса.processname);
			ТекстФильтров = СтрЗаменить(ТекстФильтров, "%2", ФильтрТекущегоСеанса.usr);
			ТекстФильтров = СтрЗаменить(ТекстФильтров, "%3", ФильтрТекущегоСеанса.sessionid);
		КонецЕсли;
		
		ФрагментТекста = СтрЗаменить("<event>
			|  <eq property=""name"" value=""%1""/>
			|%2
			|</event>", "%1", ЗначениеСобытия);
		Текст.Добавить(СтрЗаменить(ФрагментТекста, "%2", ТекстФильтров));
	КонецЦикла;
	
	Текст.Добавить("<property name=""all""/>
		|  <property name=""sql""/>
		|  <property name=""plansqltext""/>
		|</log>");
	Текст.Добавить(ШаблоныТегов.Закрывающий + " " + ШаблоныТегов.ОткрывающийОкончание);
	...
	Результат.Вставить("Текст", СтрСоединить(Текст, Символы.ПС));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Консоль запросов';
				|en = 'Query console'", КодОсновногоЯзыка());
...
Функция СобытияКонсолиЗапросов(ИнформационнаяБазаФайловая = Ложь)
	
	Результат = Новый Массив;
	Если НЕ ИнформационнаяБазаФайловая Тогда
		Результат.Добавить("db2");
		Результат.Добавить("dbmssql");
		Результат.Добавить("dbpostgrs");
		Результат.Добавить("dboracle");
	КонецЕсли; 
	Результат.Добавить("SDBL");
	Результат.Добавить("DBV8DBEng");
...
Процедура ОбойтиУзлыКонфигурацииТехЖурнала(КонфигурацияТехЖурнала, ТекущийУзел, ФильтрТекущегоСеанса, ПараметрыОбработки)
	...
	Для Каждого Узел Из ТекущийУзел.ДочерниеУзлы Цикл
		...
		Если Узел.ИмяУзла = "log" Тогда
			...
			ОбойтиУзлыКонфигурацииТехЖурнала(КонфигурацияТехЖурнала, Узел, ФильтрТекущегоСеанса, ПараметрыОбработки);
			ФильтрыСобытияТЖ = ПараметрыОбработки.ФильтрыСобытияТЖ;
			
			ВключитьТехнологическийЖурнал = (ФильтрыСобытияТЖ.ТекущееСобытие = Неопределено
				...
				И ПараметрыОбработки.ЗаменитьФильтры);
			Если ВключитьТехнологическийЖурнал Тогда
				СобытияТехнологическогоЖурнала = СобытияКонсолиЗапросов(ИнформационнаяБазаФайловая);
				Для Каждого ЭлементСобытие Из СобытияТехнологическогоЖурнала Цикл
					ДобавитьСобытиеКонсолиЗапросов(КонфигурацияТехЖурнала,Узел, ФильтрТекущегоСеанса, ЭлементСобытие);
				КонецЦикла;
			Иначе
				ВключитьТехнологическийЖурнал = (ИнформационнаяБазаФайловая
				 	И ФильтрыСобытияТЖ.sessionid = Неопределено
				  ...
				  Или (ИнформационнаяБазаФайловая
				 	...
				 	И ФильтрыСобытияТЖ.ТекущаяБаза = Неопределено
				 	И ФильтрыСобытияТЖ.ТекущийПользователь = Неопределено)
				  Или (НЕ ИнформационнаяБазаФайловая
					И ФильтрыСобытияТЖ.ТекущееСобытие <> Неопределено
					...
					И ФильтрыСобытияТЖ.ТекущийПользователь = ФильтрТекущегоСеанса.usr);
			КонецЕсли;
			Если ВключитьТехнологическийЖурнал Тогда
...
Процедура НайтиУзлыКУдалению(ТекущийУзел,ФильтрТекущегоСеанса,УзлыКУдалению, КоличествоEvent = 0, ФильтрыСобытияТЖ = Неопределено)
	...
	Для Каждого Узел Из ТекущийУзел.ДочерниеУзлы Цикл
		...
		Если Узел.ИмяУзла = "event" Тогда
			...
			УзелКУдалению = (ИнформационнаяБазаФайловая
				 И ФильтрыСобытияТЖ.ТекущееСобытие <> Неопределено
				 И ФильтрыСобытияТЖ.sessionid = Неопределено
				 И ФильтрыСобытияТЖ.ТекущаяБаза = ФильтрТекущегоСеанса.DataBase
				 И ФильтрыСобытияТЖ.ТекущийПользователь = Неопределено) 
				...
				Или (НЕ ИнформационнаяБазаФайловая
				 ...
				 И СеансКонсолиЗапросовНеактивен(ФильтрТекущегоСеанса, ФильтрыСобытияТЖ));
			Если УзелКУдалению Тогда
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение СтрЗаменить(НСтр("ru = 'Общий модуль ""%1"" не найден.';
											|en = 'Common module %1 is not found.'"), "%1", Имя);
	КонецЕсли;
	...
	Для каждого Поле Из СоответствиеБДИндекс Цикл
		...
		Если СтрНайти(ПланЗапросаВМетаданных, Поле.Ключ) Тогда
			...
			Иначе
				Ключ = Поле.Ключ;
			КонецЕсли;
			ПланЗапросаВМетаданных = СтрЗаменить(ПланЗапросаВМетаданных, Ключ, НСтр("ru = 'Индекс по';
																					|en = 'Index by'") + " " + Поле.Значение + "");
		КонецЕсли;
