...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.КодПользователя) Тогда
	...
	Если ЗначениеЗаполнено(Параметры.Содержание) Тогда
		ТекстСообщения = ЭкранироватьСимволы(Параметры.Содержание);
	КонецЕсли;
		
	Если ЗначениеЗаполнено(Параметры.Вложения) Тогда
		Для Каждого Вложение Из Параметры.Вложения Цикл
			Файл = Новый Файл(Вложение.ИмяФайла);
			ПоместитьФайлыБезРасширенияНаСервере(Вложение.Данные, Новый Структура("Имя, Расширение", Файл.ИмяБезРасширения, Файл.Расширение));
		КонецЦикла;
	КонецЕсли;
	
	ЗаполнитьСодержание(ТекстСообщения);
...
&НаСервере
Функция ЭкранироватьСимволы(Знач Текст)
...
&НаСервере
Процедура ЗаполнитьСодержание(ТекстСообщения)
...
Процедура СоздатьЭлементыФормыДляВложенногоФайла()
	...
	Для Каждого ВыбираемыйФайл Из ВыбираемыеФайлы Цикл
		...
		ТекстИмениФайла.Заголовок = СтроковыеФункции.ФорматированнаяСтрока("<a href=""%1"">%2</a>", Строка(Индекс), ПредставлениеФайла);
