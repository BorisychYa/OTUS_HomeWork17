	...
	Если ЗначениеЗаполнено(КаталогВыгрузки) Тогда
		...
		Если Не Каталог.Существует() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указанный каталог выгрузки ""%1"" не существует.';
					|en = 'Specified ""%1"" export directory does not exist.'"), КаталогВыгрузки);
		КонецЕсли;
		Если НайтиФайлы(КаталогВыгрузки, "Configuration.xml").Количество() = 0 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указанный каталог выгрузки ""%1"" не содержит файлов выгрузки конфигурации.';
					|en = 'Specified ""%1"" export directory does not contain configuration export files.'"), КаталогВыгрузки);
		КонецЕсли;
		Возврат;
	КонецЕсли;
