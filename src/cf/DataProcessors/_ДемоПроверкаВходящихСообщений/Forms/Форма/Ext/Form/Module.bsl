...
Процедура ПроверитьВходящиеВыполнитьЗавершение()
	...
	Состояние(НСтр("ru = 'Загрузка входящих сообщений.';
					|en = 'Importing incoming messages.'"),,НСтр("ru = 'Пожалуйста, подождите...';
																|en = 'Please wait…'"));
	Попытка
		ЗагрузитьВходящиеСообщения();
		НовыхПисем = ВходящиеСообщения.Количество();
		Если НовыхПисем > 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Получено новых писем:';
											|en = 'You''ve got new mails:'") + " " + НовыхПисем);
		Иначе
	...
	Исключение
		РаботаСПочтовымиСообщениямиКлиент.СообщитьОбОшибкеПодключения(УчетнаяЗаписьВходящие, 
			НСтр("ru = 'Загрузка входящих сообщений';
				|en = 'Importing incoming messages'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура ЗагрузитьВходящиеСообщения()
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка входящих сообщений';
										|en = 'Check incoming messages'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , УчетнаяЗаписьВходящие,
