...
Функция Подключаемый_СтраницаДва_ПриПереходеДалее(Отказ)
	...
	Если СУсловиямиОзнакомлен Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выполняется обработчик %1 страницы № 2';
				|en = 'The %1 handler of page 2 is running.'"), "ПриПереходеДалее"));
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Сначала ознакомьтесь с условиями.';
														|en = 'First read the conditions.'"),, "СУсловиямиОзнакомлен");
		Отказ = Истина;
	КонецЕсли;
...
Функция Подключаемый_СтраницаДва_ПриПереходеНазад(Отказ)
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выполняется обработчик %1 страницы № 2';
			|en = 'The %1 handler of page 2 is running.'"), "ПриПереходеНазад"));
...
Функция Подключаемый_СтраницаДва_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выполняется обработчик %1 страницы № 2';
			|en = 'The %1 handler of page 2 is running.'"), "ПриОткрытии"));
