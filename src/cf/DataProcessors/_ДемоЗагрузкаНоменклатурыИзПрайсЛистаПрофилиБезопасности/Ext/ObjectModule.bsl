...
Процедура ПрочитатьНоменклатуру_ADODB(ПолноеИмяФайла, ЗагружаемаяНоменклатура)
	...
	Исключение
		ВызватьИсключение НСтр("ru = 'Не удалось подключить объект ADODB.
			...
			|
			|Technical information:'") + Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Процедура ЗагрузитьПрайс(ЗагружаемаяНоменклатура)
	...
	Сообщение.Сообщить();
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = СтрЗаменить(НСтр("ru = 'Загружено позиций: %1';
										|en = 'Items imported: %1'"), "%1", СозданоЭлементов + ОбновленоЭлементов);
	Сообщение.Сообщить();
