...
//    ПропуститьПредупрежденияПриОбновлении - Булево - если Истина, указать параметр "-force".
//
// Возвращаемое значение:
//  Строка - если пустая, то обновление прошло успешно,
//           в противном случае, текст предупреждений.
//
Функция ОбновитьНаИсправительнуюВерсию(ПутьКФайлуОбновления, ПропуститьПредупрежденияПриОбновлении = Истина) Экспорт
	...
	Если ПропуститьПредупрежденияПриОбновлении Тогда
		КомандаЗапуска.Добавить("-force");
	КонецЕсли;
	КомандаЗапуска.Добавить("-ClearUnresolvedRefs");
		...
		Если ПустаяСтрока(Сообщения) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось выполнить сравнение/объединение с конфигурацией из файла по причине:
				           ...
				           |Return code: %1'"), Результат.КодВозврата);
		КонецЕсли;
	КонецЕсли;
	УдалитьФайлы(ИмяФайлаСообщений);
	
	ЗначимыеСообщения = ЗначимыеСообщения(Сообщения);
	Если ПропуститьПредупрежденияПриОбновлении
	 Или ЗначениеЗаполнено(ЗначимыеСообщения)
	 Или Не ЗначениеЗаполнено(Сообщения) Тогда
		Возврат ЗначимыеСообщения;
	КонецЕсли;
	
	Возврат ОбновитьНаИсправительнуюВерсию(ПутьКФайлуОбновления);
...
Функция ЗначимыеСообщения(Сообщения)
	
	Строки = Новый Массив;
	ОбщийЗаголовок = НСтр("ru = 'Невозможно выполнение обновления конфигурации в командном режиме';
							|en = 'Cannot update the configuration in command mode'");
	ЗаголовокНеЗначимыхПредупреждений = НСтр("ru = 'Неизмененные свойства:';
											|en = 'Unchanged properties:'");
	
	НайденЗаголовокНеЗначимыхПредупреждений = Ложь;
	
	КоличествоСтрок = СтрЧислоСтрок(Сообщения);
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		Строка = СтрПолучитьСтроку(Сообщения, НомерСтроки);
		Если Строка = ЗаголовокНеЗначимыхПредупреждений Тогда
			НайденЗаголовокНеЗначимыхПредупреждений = Истина;
			Продолжить;
		КонецЕсли;
		Если НайденЗаголовокНеЗначимыхПредупреждений Тогда
	...
	Если Строки.Количество() = 1
	   И Строки[0] = ОбщийЗаголовок Тогда
		Строки = Новый Массив;
	КонецЕсли;
...
Процедура УстановитьФлажкиПодсистем(ИмяФайлаЗахвата)
	...
	Для Каждого СвойстваПодсистемы Из ВнедренныеПодсистемы(ИмяФайлаЗахвата) Цикл
		УстановитьФлажкиПодсистемы(СвойстваПодсистемы, Ложь);
		УстановитьФлажкиПодсистемы(СвойстваПодсистемы, Истина);
	КонецЦикла;
	
КонецПроцедуры

Процедура УстановитьФлажкиПодсистемы(СвойстваПодсистемы, ВтораяКонфигурация)
	
	УзелОбъект = ДокументDOM.СоздатьЭлемент("Object");
	Если ВтораяКонфигурация Тогда
		УзелОбъект.УстановитьАтрибут("fullNameInSecondConfiguration", СвойстваПодсистемы.Ключ);
	Иначе
		УзелОбъект.УстановитьАтрибут("fullName", СвойстваПодсистемы.Ключ);
	КонецЕсли;
	...
	Иначе
		УзелПодсистема.УстановитьАтрибут("configuration", "Main");
	КонецЕсли;
...
Процедура УстановитьФлажкиРазделителей()
	...
	Для Каждого Разделитель Из Разделители Цикл
		УстановитьПравилоДляОбъектаИСвойств(Разделитель,
			Новый Структура("Состав", "MergePrioritizingSecondConfiguration"));
	КонецЦикла;
КонецПроцедуры
...
Процедура УстановитьФлажкиОпределяемыхТипов()
	...
	Для Каждого СвойстваТипа Из ОпределяемыеТипы() Цикл
		...
		УстановитьПравилоДляСвойства(УзелСвойства, "Тип", Правило);
...
Процедура ДобавитьПодсистемы(Подсистемы, МетаданныеПодсистемы, Запись)
	...
	Для Каждого Подсистема Из МетаданныеПодсистемы.Подсистемы Цикл
		Если Подсистема.Подсистемы.Количество() = 0 Или Подсистема.Имя = "ОбменДанными" Тогда
