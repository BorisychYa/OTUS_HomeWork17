...
Процедура ОчиститьДанныеИнформационнойБазы(УдалятьРасширения) Экспорт
	...
	Если ОписаниеБТС = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В конфигурацию не внедрена библиотека ""1С:Библиотека технологии сервиса"".
                  ...
                  |
                  |To use the configuration in SaaS, integrate 1C:Cloud Technology Library version %1 or later.'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ТребуемаяВерсияБТС());
...
Функция ИмяМетодаПодготовитьОбластьИзВыгрузки() Экспорт
	
	Возврат "РаботаВМоделиСервисаБТС.ПодготовитьОбластьИзВыгрузки";
...
Процедура УдалитьПользователейОбласти()
