...
Процедура ЗагрузитьФайл(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Поле ""Куда сохранять"" не заполнено';
				|en = 'The ""Where to save"" field is blank'"), , "КудаСохранять");
	КонецЕсли;
...
#Иначе
	...
	Если НЕ ЗначениеЗаполнено(АдресДляЗагрузки) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Поле ""Что загружать"" не заполнено.';
														|en = 'The ""What to import"" field is blank'"),, "АдресДляЗагрузки");
		ВыполнитьОбработкуОповещения(Оповещение, Неопределено);
		Возврат;
	КонецЕсли;
