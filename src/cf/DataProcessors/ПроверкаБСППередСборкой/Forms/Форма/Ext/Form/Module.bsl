...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьПроверку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|to view in role %1<%2>'"), "Подсистема", "ИмяПодсистемы"),
		"Подключаемый_ПроверкаПраваПросмотрПодсистемКомандногоИнтерфейса()");
	
	ДобавитьПроверку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Недопустимо наличие ""%1"" в текстах модулей';
...
Процедура ВыполнитьПроверку(СписокВывода = Неопределено) Экспорт
	...
	Для Каждого Проверка Из Объект.Проверки Цикл
		Если Проверка.ПроводитьПроверку Тогда
			Попытка
				Состояние(НСтр("ru = 'Проверка правила:';
								|en = 'Rule check:'") + " " + Проверка.НаименованиеПроверки);
			...
			Исключение
				...
				ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru = 'Проверка перед сборкой';
																					|en = 'Check before assembly'"), "Ошибка", ПредставлениеОшибки);
...
Функция Подключаемый_ПроверитьВхождениеРолейИВидовДоступаВПрофили()
	...
	Для Каждого СвойстваВидаДоступа Из ВидыДоступаМассив Цикл
		...
		СписокОшибочныхОбъектов.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вид доступа ""%1"".';
				|en = 'Access kind %1.'"), ИмяВидаДоступа));
	КонецЦикла;
	...
	Для Каждого Роль Из Метаданные.Роли Цикл
		...
		СписокОшибочныхОбъектов.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Роль ""%1"".';
				|en = 'Role %1.'"), ИмяРоли), Роль.ПолноеИмя());
	КонецЦикла;
	...
	ОписаниеИсправления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Это означает, что работа приложения с этой ролью (видом доступа) не тестируется и не демонстрируется в демо-базе.
			|Для исправления: 
...
Функция Подключаемый_ПроверитьСовпадениеМодулейУправляемогоИОбычногоПриложений()
	...
	Если ТекстФайлаОбычногоПриложения = Неопределено Или ТекстФайлаУправляемогоПриложения = Неопределено Тогда
		...
		Возврат Новый Структура("ПредставлениеОшибки, ОписаниеИсправления, ОшибочныеДанные",НСтр("ru = 'Не удалось найти файлы модулей приложения.';
																								|en = 'Cannot find the application module files.'"),"", Список) ;
	КонецЕсли;
	...
	Пока НачалоФрагмента > 0 Цикл
		...
		Если ПозицияКонцаФрагмента = 0 Тогда
			...
			ТекстСообщения = НСтр("ru = '[ИмяФайлаМодуля]: для открывающей скобки [НачалоФрагмента] не обнаружена закрывающая скобка.';
									|en = '[ИмяФайлаМодуля]: close parenthesis is not found for open parenthesis [НачалоФрагмента].'");
		...
		Если ПозицияКонцаФрагмента < НачалоФрагмента Тогда
			...
			ТекстСообщения = НСтр("ru = '[ИмяФайлаМодуля]: для открывающей скобки [НачалоФрагмента] закрывающая скобка расположена выше по тексту.';
									|en = '[ИмяФайлаМодуля]: close parenthesis is above in the text for open parenthesis [НачалоФрагмента].'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "[НачалоФрагмента]", "// " + ИмяПодсистемы);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "[ИмяФайлаМодуля]", ПолноеИмяСтруктура.ПолноеИмяМодуля);
			Ошибки.Добавить(ТекстСообщения, ПолноеИмяСтруктура.ПолноеИмяОбъекта);
			Возврат;
		...
		Если НайтиНачалоФрагмента(ПромежуточнаяСтрока, ИмяПодсистемы) > 0 Тогда 
			...
			ТекстСообщения = НСтр("ru = '[ИмяФайлаМодуля]: внутри открывающейся скобки [НачалоФрагмента] есть еще одна открывающаяся скобка, до закрывающейся. Фрагмент кода: [ФрагментКода]';
									|en = '[ИмяФайлаМодуля]: there is an open parenthesis inside open parenthesis [НачалоФрагмента] before the close parenthesis. Code snippet: [ФрагментКода]'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "[НачалоФрагмента]", "// " + ИмяПодсистемы);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "[ИмяФайлаМодуля]", ПолноеИмяСтруктура.ПолноеИмяМодуля);
...
Функция НекорректныеСвязиМеждуПодсистемами()
	...
	КопияТаблицы.Свернуть("ВызывающийОбъект");
	Для Каждого СтрокаВызывающийОбъект Из КопияТаблицы Цикл
		
		ВызывающийОбъектЧастями = СтрРазделить(СтрокаВызывающийОбъект.ВызывающийОбъект, ".");
		ВызывающийОбъектЧастямиНовый = Новый Массив;
		Если ВызывающийОбъектЧастями.Количество() > 2 Тогда
			ВызывающийОбъектЧастямиНовый.Добавить(ВызывающийОбъектЧастями[0]);
			ВызывающийОбъектЧастямиНовый.Добавить(ВызывающийОбъектЧастями[1]);
			ПолноеИмяОбъекта = СтрСоединить(ВызывающийОбъектЧастямиНовый, ".");
		Иначе
			ПолноеИмяОбъекта = СтрокаВызывающийОбъект.ВызывающийОбъект;
		КонецЕсли;
		ПолноеИмяОбъекта = СтрЗаменить(СтрокаВызывающийОбъект.ВызывающийОбъект, ".Модуль", "");
		
		ПараметрыОтбора = Новый Структура("ВызывающийОбъект", СтрокаВызывающийОбъект.ВызывающийОбъект);
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если СформированнаяСтрока = ""
				...
				Если ЗначениеЗаполнено(СформированнаяСтрока) Тогда
					СписокОшибочныхОбъектов.Добавить(СформированнаяСтрока, ПолноеИмяОбъекта);
					СформированнаяСтрока = "";
				КонецЕсли;
...
Функция СвойствоРоли(Текст, ОбъектXDTO, ИмяСвойства, ТребуемоеЗначение = Неопределено)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неизвестное свойство роли:';
	...
	Если Значение = Неопределено Тогда
		Текст = Текст + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Свойство ""%1"" не найдено.';
				|en = 'The %1 property is not found.'"), ПредставлениеСвойства) + Символы.ПС;
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ТипЗнч(Значение) <> Тип("Булево") Тогда
		Текст = Текст + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Свойство ""%1"" не типа Булево.';
				|en = 'The %1 property is not Boolean.'"), ПредставлениеСвойства) + Символы.ПС;
		Возврат Неопределено;
	КонецЕсли;
		...
		Если ТребуемоеЗначение Тогда
			Текст = Текст + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Свойство ""%1"" должно быть установлено.';
					|en = 'The %1 property must be set.'"), ПредставлениеСвойства) + Символы.ПС;
		Иначе
			Текст = Текст + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Свойство ""%1"" должно быть снято.';
					|en = 'The %1 property must be cleared.'"), ПредставлениеСвойства) + Символы.ПС;
		КонецЕсли;
	КонецЕсли;
...
Функция ИмяОбъектаМетаданного(Знач ОбъектМетаданных)
	РасширениеКонфигурации = ОбъектМетаданных.РасширениеКонфигурации(); // РасширениеКонфигурации
		ИмяКонфигурации = ?(РасширениеКонфигурации <> Неопределено, 
			НСтр("ru = 'расширение';
				|en = 'extension'") + " " + РасширениеКонфигурации.Имя, 
			НСтр("ru = 'конфигурация';
				|en = 'configuration'") + " " + Метаданные.Имя);
