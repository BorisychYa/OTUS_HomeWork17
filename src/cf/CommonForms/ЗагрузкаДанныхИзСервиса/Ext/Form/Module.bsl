...
Процедура ОткрытьВыборФайла(Заголовок)
	...
	ПараметрыПередачи.ФильтрДиалогаВыбора = СтрШаблон(НСтр("ru = 'Архивы %1';
...
Функция ПроверитьКонфигурациюВыгрузкиВнутр(ЧтениеДанныхАрхива)
	...
	Если ПутьКФайлуСодержимого = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'В архиве отсутствует файл содержимого (%1)';
...
Процедура УдалитьФайлыВПопытке(ИмяПапкиИлиФайла)
	...
	ИмяСобытияЖР = НСтр("ru = 'Удаление файла.Загрузка файла выгрузки';
						|en = 'Delete file.Import export file'", ОбщегоНазначения.КодОсновногоЯзыка());
