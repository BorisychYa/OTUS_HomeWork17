		   ...
		   И ВидДоступа <> "ПравоИзменения" Тогда
			...
			Если ВидДоступа = "Объект" Или ВидДоступа = "НастройкиПрав" Тогда
				...
				Для Каждого Тип Из Запрос.Выполнить().Выгрузить().Колонки.ПолеИскомыхТипов.ТипЗначения.Типы() Цикл
					...
					// так как этот инструмент разработчика неотделим от подсистемы, к которой принадлежит объект.
...
Процедура ДобавитьПраваРоли(Роль, Контекст, ЕстьОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Права роли %1 не прочитаны по причине:';
			|en = 'Cannot read the %1 role access rights due to:'"), Роль);
	...
	Пока Не (  ЧтениеXML.Имя = "Rights"
		...
		Иначе
			ЗаписатьСообщение(Контекст.Журнал, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось прочитать файл прав, т.к. обнаружен неизвестный элемент %1';
					|en = 'Cannot read the rights file as an unknown item %1 is found'"), ЧтениеXML.Имя));
			ЕстьОшибки = Истина;
			Возврат;
		КонецЕсли;
...
Процедура ПрочитатьОбъект(ЧтениеXML, Контекст, ЗаголовокОшибки, ЕстьОшибки)
	...
	Если ИмяОбъекта = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено свойство %1 (имя объекта)';
...
Функция ОтправитьТекстыОграничений(ПараметрыОтправки)
	...
	ИмяСобытияВЖурналеРегистрации = НСтр("ru = 'Контроль изменения текстов RLS';
										|en = 'Control of RLS text change'", ОбщегоНазначения.КодОсновногоЯзыка());
	ШаблонСообщенияОбОшибке = НСтр("ru = 'Отправка текстов ограничений не выполнена по причине:';
									|en = 'Restriction texts are not sent due to:'") + Символы.ПС + "%1";
	...
	Если HTTPОтвет.КодСостояния <> 200 Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщенияОбОшибке, НСтр("ru = 'Код состояния:';
											|en = 'Status code:'")) + " " + HTTPОтвет.КодСостояния + ?(ЗначениеЗаполнено(Пояснение), Символы.ПС + Пояснение, "");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
