...
Процедура НаписатьЭлектронноеПисьмо(Команда)
	...
	ПараметрыПисьма.Представление = ПредставлениеЭлектроннойПочты;
	ПараметрыПисьма.ОжидаемыйВид = ВидКонтактнойИнформацииЭлектроннойПочты;
	ПараметрыПисьма.ИсточникКонтактнойИнформации = Объект.Партнер;
	ПараметрыПисьма.ИмяРеквизита = "ПредставлениеЭлектроннойПочты";
	УправлениеКонтактнойИнформациейКлиент.СоздатьЭлектронноеПисьмо(Объект.ЭлектроннаяПочта, ПараметрыПисьма);
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Неудачная отправка оповещения';
				|en = 'Failed to send notification'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбновитьСчетчикиСтрокТаблиц()
	...
	Элементы.СтраницаСчетаНаОплату.Заголовок = 
		?(Объект.СчетаНаОплату.Количество() > 0, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Счета на оплату (%1)';
																		|en = 'Proforma invoices (%1)'"), Объект.СчетаНаОплату.Количество()),
	...
	Элементы.СтраницаПартнерыИКонтактныеЛица.Заголовок = 
		?(Объект.ПартнерыИКонтактныеЛица.Количество() > 0, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Контактные лица (%1)';
																		|en = 'Contact persons (%1)'"), Объект.ПартнерыИКонтактныеЛица.Количество()),
