...
Процедура СписокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.ВыборЕдинственного = Истина;
	ПараметрыФормы.НачальноеЗначениеВыбора = Список;
	ПараметрыФормы.КоллекцииВыбираемыхОбъектовМетаданных = Коллекции;
	
	СтандартныеПодсистемыКлиент.ВыбратьОбъектыМетаданных(ПараметрыФормы, 
		Новый ОписаниеОповещения("СписокНачалоВыбораЗавершение", ЭтотОбъект));
...
Процедура ОбновитьДоступ(Команда)
	...
	УправлениеДоступомСлужебныйКлиент.ОткрытьФормуОбновлениеДоступаНаУровнеЗаписей(Истина, Истина);
...
#Если Не ВебКлиент Тогда
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Контроль изменения текстов RLS';
										|en = 'Control of RLS text change'"), , НСтр("ru = 'Сравнение текстов RLS подготовлено.';
	...
	ИначеЕсли Свойства.ОграничениеПоВладельцуВозможно = Истина Тогда
		...
		НадписьОграничениеПоВладельцуБезОптимизации.Заголовок =
			?(Свойства.ПоВладельцуБезЗаписиКлючейДоступа = Ложь,
				НСтр("ru = 'Без оптимизации (отключена)';
					|en = 'No optimization (disabled)'"), НСтр("ru = 'Без оптимизации';
																|en = 'No optimization'"));
	КонецЕсли;
		...
		Если МетаданныеТаблицы = Неопределено Тогда
			ДобавитьОшибку(ОшибкиВВыгрузке, НомерСтроки, ЕстьОшибка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти объект метаданных ""%1"".';
					|en = 'Cannot find the ""%1"" metadata object.'"), Свойства.Таблица));
		КонецЕсли;
		
		Если МетаданныеРоли = Неопределено Тогда
			ДобавитьОшибку(ОшибкиВВыгрузке, НомерСтроки, ЕстьОшибка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти роль ""%1"".';
					|en = 'Cannot find role %1.'"), Свойства.Роль));
		КонецЕсли;
		
		Если Права.Получить(ВРег(Свойства.Право)) = Неопределено Тогда
			ДобавитьОшибку(ОшибкиВВыгрузке, НомерСтроки, ЕстьОшибка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректное имя права ""%1"".';
					|en = 'Incorrect %1 right name.'"), Свойства.Право));
		КонецЕсли;
...
Процедура СоздатьНовуюИБ(ИмяПодключения, ИмяФайлаШаблона = "")
	...
	Если Отказ Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Строка соединения ИБ ""%1"" указана неверно:';
				|en = 'Infobase connection string ""%1"" is specified incorrectly:'"), ИмяПодключения) + Символы.ПС
...
#Иначе
	...
	Если КодВозврата <> 0 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Операция не выполнена.';
			...
			+ НСтр("ru = 'Строка запуска:';
					|en = 'Launch string:'") + Символы.ПС 
			+ СтрокаЗапуска + Символы.ПС 
			+ НСтр("ru = 'Код возврата:';
					|en = 'Return code:'")+ " " + КодВозврата + Символы.ПС
			+ НСтр("ru = 'Содержимое лога';
					|en = 'Log entries'") + " " + ИмяФайлаЛога + ":" + Символы.ПС
			+ ЛогЗапуска;
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция НастройкиПодключения(ИмяПодключения)
	...
	Если НастройкиПодключения = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Настройки подключения %1 не найдены.';
				|en = 'Connection settings %1 are not found.'"), ИмяПодключения);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
