		...
		ИначеЕсли Выборка.СтатусТранзакции = Перечисления.СтатусыТранзакцииСРеестромМЧД.Ошибка
			...
			ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<span style=""color: %1"">: <a href = ""%2"">Не удалось отменить</a>';
					|en = '<span style=""color: %1"">: <a href = ""%2"">Revokation failed</a>'"), "ПоясняющийТекст", "ПоказатьОшибку");
...
Функция ПолныеДанныеДоверенностиМЧДРР(НомерДоверенности, ИННДоверителя, ИННПредставителя, ТокенДоступа = "", ЦельПолучения = "")
	...
	Если Не ЗначениеЗаполнено(Результат.СтатусПолучения) Тогда
		ТекстОшибки = НСтр("ru = 'Распределенный реестр ФНС России не предоставил полных данных доверенности и не сообщил о статусе запроса данных.';
							|en = 'Распределенный реестр ФНС России не предоставил полных данных доверенности и не сообщил о статусе запроса данных.'");
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Распределенный реестр ФНС России не предоставил полных данных доверенности, статус запроса данных: %1 (%2).';
				|en = 'Распределенный реестр ФНС России не предоставил полных данных доверенности, статус запроса данных: %1 (%2).'"),
	...
	Если ЗначениеЗаполнено(Результат.СтатусПолучения) Тогда
		Результат.Ошибка.Решение = СтроковыеФункции.ФорматированнаяСтрока(
			...
			|en = 'Повторите действие позднее или скачайте архив доверенности с <a href=%1>сайта распределенного реестра ФНС России</a> и загрузите из файла с помощью команды в <a href=%2>списке машиночитаемых доверенностей</a>.
			|При необходимости обратитесь <a href=%3>службу поддержки ФНС России</a>.'"),
