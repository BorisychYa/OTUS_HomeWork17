...
Процедура Подключаемый_БизнесПроцессыИЗадачи_ПроверитьВнедрение()
	...
	Если МетаданныеОбъектовАдресацииЗадач.Количество() <> 1 И ТипыОбъектовАдресации.Количество() <> 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Типы предопределенных элементов ПВХ %1';
				|en = 'Predefined item types of CCT %1'"), "ОбъектыАдресацииЗадач");
...
Процедура Подключаемый_ДатыЗапретаИзменения_ПроверитьВнедрение()
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректно заполнена процедура %1';
				|en = 'Procedure %1 is filled in incorrectly'"), "ПриЗаполненииРазделовДатЗапретаИзменения");
	...
	КраткоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Некорректно заполнена процедура %1';
			|en = 'Procedure %1 is filled in incorrectly'"), "ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения");
...
Процедура Подключаемый_ИнтерфейсOData_ПроверитьВнедрение()
	...
	Если ОшибкиРоли.Количество() > 0 Тогда
		...
		ТекстОшибкиКратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректный состав прав роли %1.';
				|en = 'Role ""%1"" has invalid list of rights.'"), Роль.Имя);
		...
		Если ИсправлятьОшибку("СоставРолиOData") Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Исправлено. Изменен состав прав роли %1.';
					|en = 'Fixed. Role rights composition changed%1.'"), Роль.Имя);
...
Процедура Подключаемый_КонтактнаяИнформация_ПроверитьВнедрение()
	...
	Для каждого СтрокаТаблицы Из ДанныеНачальногоЗаполнения.ПредопределенныеДанные Цикл
			...
			Если ПроверкаНаДубли.Получить(СтрокаТаблицы.ИмяПредопределенныхДанных) = Неопределено Тогда
				...
				Если ПредопределенныйВид = Неопределено Тогда
					...
					НСтр("ru = 'Предопределенный вид контактной информации %1 описан в коде начального заполнения,
						...
						|en = 'The predefined contact information kind ""%1"" is described in the initial population code
						|but is missing from the infobase. You should add it manually.
			...
			Если ПроверкаНаДубли.Получить(СтрокаТаблицы.ИмяПредопределенногоВида) = Неопределено Тогда
				...
				Если ПредопределенныйВид = Неопределено Тогда
					...
					НСтр("ru = 'Предопределенный вид контактной информации %1 описан в процедуре %2 кода начального заполнения,
						...
						|en = 'The predefined contact information kind ""%1"" is described in the initial population procedure %2
						|but is missing from the infobase. You should add it using the update handler.
						|For details, see the ""%3"" subsystem in 1C:SSL documentation: %4'"),
		...
		Иначе
			...
			НСтр("ru = 'В процедуре %1 кода начального заполнения описан предопределенный вид контактной информации %2
				...
				|For details, see the ""%4"" subsystem in 1C:SSL documentation: %5'"),
	...
	Для Каждого ПредопределенныйВид Из ВидыКонтактнойИнформации Цикл
		...
		Иначе
			...
			|Подробнее см. документацию по внедрению подсистемы ""%3"" 1С:Библиотеки стандартных подсистем: %4';
			|en = 'Contact information kind ""%1"" is present in the infobase but is missing from
			...
			ДобавитьОшибку(МетаданныеВидовКонтактнойИнформации, 
				НСтр("ru = 'В код начального заполнения отсутствует описание переопределенного вида контактной информации.';
					|en = 'The initial population code is missing the details of a redefined contact information kind.'"), ТекстОшибки);
	...
	МассивТипов.Добавить(СоставТипа(НСтр("ru = 'Отсутствует предопределенная группа справочника Виды контактной информации.';
										|en = 'The ""Contact information kinds"" catalog is missing a predefined group.'"), ОбъектыВладельцы));
	...
	Для Каждого ПроверяемыйОбъект Из ТаблицаВидов Цикл
		...
		Если ТабличнаяЧастьКонтактнаяИнформация = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У объекта ""%1"", подключенного к подсистеме отсутствует
				...
				|'"), МетаданныеВладельца.ПолноеИмя(), "КонтактнаяИнформация");
				
			ДобавитьОшибку(МетаданныеВладельца, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отсутствует табличная часть %1';
					|en = 'The %1 table is missing'"), "КонтактнаяИнформация"), ТекстОшибки);
			Продолжить;
		КонецЕсли;
		...
		Для Каждого ТипТабличнойЧасти Из ТипыТабличнойЧасти Цикл
			...
			Если НайденныйРеквизит = Неопределено Тогда
				Если ОбязательныйРеквизит Тогда
					...
					|in table ""%2""'"), МетаданныеВладельца.ПолноеИмя(), "КонтактнаяИнформация", ТипТабличнойЧасти.Ключ);
					ДобавитьОшибку(МетаданныеВладельца, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Проверка наличия обязательных реквизитов табличной части %1';
							|en = 'Check if the %1 table contains the required attributes'"), "КонтактнаяИнформация"), ТекстОшибки);
				КонецЕсли;
			Иначе
				...
				Если РеквизитыСПолнотекстовымПоиском[ТипТабличнойЧасти.Ключ] = Истина Тогда
					...
					Если НайденныйРеквизит.ПолнотекстовыйПоиск = Метаданные.СвойстваОбъектов.ИспользованиеПолнотекстовогоПоиска.НеИспользовать Тогда
						...
						|in table ""%2"" must have full-text search enabled.'"), МетаданныеВладельца.ПолноеИмя(), "КонтактнаяИнформация", ТипТабличнойЧасти.Ключ);
						ДобавитьОшибку(МетаданныеВладельца, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Проверка настроек полнотекстового поиска у реквизитов табличной части %1';
								|en = 'Checking full-text search settings of attributes of the %1 table'"), "КонтактнаяИнформация"), ТекстОшибки);
					КонецЕсли;
				...
				ИначеЕсли НайденныйРеквизит.ПолнотекстовыйПоиск = Метаданные.СвойстваОбъектов.ИспользованиеПолнотекстовогоПоиска.Использовать Тогда
					...
					|en = 'Attribute ""%3"" of object ""%1""
					|in table ""%2"" must have full-text search disabled.'"), МетаданныеВладельца.ПолноеИмя(), "КонтактнаяИнформация", ТипТабличнойЧасти.Ключ);
					ДобавитьОшибку(МетаданныеВладельца, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Проверка настроек полнотекстового поиска у реквизитов табличной части %1';
							|en = 'Checking full-text search settings of attributes of the %1 table'"), "КонтактнаяИнформация"), ТекстОшибки);
				...
				Если НайденныйРеквизит.Тип <> ТипТабличнойЧасти.Значение Тогда
					...
					|in table ""%2"" does not match the expected type'"), МетаданныеВладельца.ПолноеИмя(), "КонтактнаяИнформация", ТипТабличнойЧасти.Ключ);
					ДобавитьОшибку(МетаданныеВладельца, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Некорректные типы реквизитов табличной части %1';
							|en = 'Incorrect types of attributes of the %1 table'"), "КонтактнаяИнформация"), ТекстОшибки);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Если ТабличнаяЧастьКонтактнаяИнформация.Реквизиты.Найти("ВидДляСписка") <> Неопределено Тогда
			...
			Если Не ХарактеристикиЗаданы Тогда
				...
				ДобавитьОшибку(МетаданныеВладельца, НСтр("ru = 'Некорректно заполнены характеристики';
														|en = 'Characteristics are filled in incorrectly'"), ТекстОшибки);
			КонецЕсли;
			Если ХарактеристикиЗаданы И Не ХарактеристикиЗаданыКорректно Тогда
				...
				ДобавитьОшибку(МетаданныеВладельца, НСтр("ru = 'Некорректно заполнены поля отборов видов характеристики';
														|en = 'Fields of the characteristic type filters are filled in incorrectly'"), ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура Подключаемый_ОбновлениеВерсииИБ_ПроверитьВнедрение()
	...
	Если Метаданные.ОбщиеМакеты.Найти("ОписаниеИзмененийСистемы") = Неопределено Тогда
		ДобавитьОшибку(Неопределено, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отсутствует макет %1';
																		|en = 'Template %1 is missing'"), "ОписаниеИзмененийСистемы"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В конфигурации не создан общий макет %1.';
																		|en = 'Common template %1 is not created in the configuration.'"), "ОписаниеИзмененийСистемы"));
	КонецЕсли;
	...
	Для Каждого ОбъектСНачальнымЗаполнением Из ОбъектыСНачальнымЗаполнением Цикл
		...
		Если ИменаПредопределенныхБезЗаполнения.Количество() > 0 Тогда
			...
			НСтр("ru = 'Для объекта ""%1"" отсутствует код заполнения предопределенных элементов.';
				|en = 'Object ""%1"" is missing population code for predefined items.'"),  МетаданныеОбъекта.ПолноеИмя());
			...
			Для Каждого Форма Из ОбъектМетаданных.Формы Цикл // ОбъектМетаданныхФорма
				...
				Если НастройкиИсточника.ВариантыОтчетов Тогда
					ПодсистемыИсточника.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Варианты отчетов (см. %1.%2)';
								|en = 'Report options (see %1.%2)'"), "ВариантыОтчетовПереопределяемый", "ОпределитьОбъектыСКомандамиОтчетов"));
				КонецЕсли;
				Если НастройкиИсточника.ДополнительныеОтчетыИОбработки Тогда
					ПодсистемыИсточника.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Дополнительные отчеты и обработки (см. состав определяемого типа %1)';
								|en = 'Additional reports and data processors (see content of the %1 type collection)'"), "ОбъектСДополнительнымиКомандами"));
				КонецЕсли;
				Если НастройкиИсточника.ЗаполнениеОбъектов Тогда
					ПодсистемыИсточника.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Заполнение объектов (см. %1.%2)';
								|en = 'Filling in objects (see %1.%2)'"), "ЗаполнениеОбъектовПереопределяемый", "ПриОпределенииОбъектовСКомандамиЗаполнения"));
				КонецЕсли;
				Если НастройкиИсточника.Печать Тогда
					ПодсистемыИсточника.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Печать (см. %1.%2)';
								|en = 'Print (see %1.%2)'"), "УправлениеПечатьюПереопределяемый", "ПриОпределенииОбъектовСКомандамиПечати"));
				КонецЕсли;
				Если НастройкиИсточника.НапоминанияПользователя Тогда
					ПодсистемыИсточника.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Напоминания пользователя (см. состав определяемых типов: %1, %2)';
								|en = 'User reminders (see type collections: %1, %2)'"), "ПредметНапоминания", "ПредметНапоминанияОбъект"));
				КонецЕсли;
				...
				Если ПодсистемыИсточника.Количество() = 1 Тогда
					ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект подключен к подсистеме %1, требующей встраивание подсистемы ""Подключаемые команды"".';
							|en = 'Объект подключен к подсистеме %1, требующей встраивание подсистемы ""Подключаемые команды"".'"), ОписаниеВставки);
				ИначеЕсли ПодсистемыИсточника.Количество() > 1 Тогда
					ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект подключен к подсистемам, требующим встраивание подсистемы ""Подключаемые команды"":
							|%1.';
							|en = 'Объект подключен к подсистемам, требующим встраивание подсистемы ""Подключаемые команды"":
							|%1.'"), ОписаниеВставки);
				КонецЕсли;
						...
						Если Присутствующие.Количество() > 0 Тогда
							Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'В форме ""%1"" отсутствуют фрагменты кода подсистемы ""Подключаемые команды"":
									|%2.';
									|en = 'The ""Attachable commands"" subsystem code snippets are missing  in the %1 form: 
									|%2.'"),
								Форма.Имя,
					...
					Если НайденнаяПроцедура <> Неопределено Тогда
						Если Не СтрЗаканчиваетсяНа(НРег(СокрЛП(НайденнаяПроцедура.Заголовок)), НРег(" Экспорт")) Тогда
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								...
								|Subsystem %3 cannot call it.'"), "Подключаемый_ВыполнитьКомандуНаСервере", 
...
Процедура Подключаемый_Пользователи_ПроверитьВнедрение()
	...
	КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка в процедуре %1 общего модуля %2.';
			|en = 'Error in procedure %1 of common module %2.'"), "ПриОпределенииНазначенияРолей", "ПользователиПереопределяемый");
...
Процедура Подключаемый_ПрефиксацияОбъектов_ПроверитьВнедрение()
	...
	Если ТипыПрефиксаОрганизации.Количество() = 0 И ТипыПрефиксаИБИОрганизации.Количество() = 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|%1.%2 
			|or %1.%3'"), "ПрефиксацияОбъектовСобытия", "УстановитьПрефиксОрганизации", "УстановитьПрефиксИнформационнойБазыИОрганизации");
		ДобавитьОшибку(Метаданные.ОбщиеМодули.ПрефиксацияОбъектовСобытия, НСтр("ru = 'Отсутствуют подписки установки префикса';
																				|en = 'Prefix setting subscriptions are missing'"), ТекстОшибки);
	КонецЕсли;
	...
	Если ТипыПрефиксаИБ.Количество() = 0 И ТипыПрефиксаИБИОрганизации.Количество() = 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Subscription to event with handler is expected:
			|%1.%2
			|or %1.%3'"), "ПрефиксацияОбъектовСобытия", "УстановитьПрефиксИнформационнойБазы", "УстановитьПрефиксИнформационнойБазыИОрганизации");
		ДобавитьОшибку(Метаданные.ОбщиеМодули.ПрефиксацияОбъектовСобытия, НСтр("ru = 'Отсутствуют подписки установки префикса';
																				|en = 'Prefix setting subscriptions are missing'"), ТекстОшибки);
	КонецЕсли;
...
Процедура Подключаемый_РаботаСФайлами_ПроверитьВнедрение()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВладельцевПрисоединенныхФайлов Цикл
		...
		Если Не ЕстьСправочникПрисоединенныхФайлов И Не ЭтоПредопределенныеПрисоединенныеФайлы Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для справочника ""%1"" не создан справочник хранения присоединенных файлов.';
																						|en = 'A catalog for storing attachments for catalog ""%1"" is not created.'"), СтрокаТаблицы.ВладелецПрисоединенныхФайлов.Имя);
			ДобавитьОшибку(СтрокаТаблицы.ВладелецПрисоединенныхФайлов.Имя, НСтр("ru = 'Ошибка создания справочника хранения присоединенных файлов.';
																				|en = 'Cannot create a catalog for storing attachments.'"), ТекстОшибки);
	...
	Для Каждого МетаданныеСправочника Из ОбъектыСФайлами Цикл
		...
		Если ТипыПрисоединенныхФайлов.Найти(ТипЗначенияСправочник) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Справочник ""%1"", содержащий присоединенные файлы,
				...
				|is not included in the %2 type collection.'"), ИмяСправочника, "ПрисоединенныйФайл");
			ДобавитьОшибку(МетаданныеСправочника, НСтр("ru = 'Справочник не включен в определяемый тип';
														|en = 'Catalog is not included in the type collection'"), ТекстОшибки);
		КонецЕсли;
		...
		Если ТипыПрисоединенныхФайловОбъект.Найти(ТипЗначенияСправочникОбъект) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Справочник ""%1"", содержащий присоединенные файлы,
				|не включен в определяемый тип ""%2"".';
				|en = 'Catalog ""%1"" that contains attachments
				|is not included in the %2 type collection.'"), ИмяСправочника, "ПрисоединенныйФайлОбъект");
			ДобавитьОшибку(МетаданныеСправочника, НСтр("ru = 'Справочник не включен в определяемый тип';
														|en = 'Catalog is not included in the type collection'"), ТекстОшибки);
		КонецЕсли;
		...
		Для Каждого ТипВладельца Из ТипыВладельцев Цикл
			...
			Если Не ОбщегоНазначения.ЭтоСсылка(ТипВладельца) Тогда
				...
				ДобавитьОшибку(МетаданныеСправочника, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Некорректный тип реквизита ""%1""';
						|en = 'Incorrect type of attribute ""%1""'"), "ВладелецФайла"), ТекстОшибки);
			ИначеЕсли ВладельцыФайлов.Найти(ТипВладельца) = Неопределено Тогда
				...
				Если ТипыВладельцевПрисоединенныхФайлов.Найти(ТипВладельца) = Неопределено Тогда
					...
					ДобавитьОшибку(МетаданныеСправочника, НСтр("ru = 'Владелец не включен в определяемый тип';
																|en = 'Owner is not included in the type collection'"), ТекстОшибки);
				КонецЕсли;
	...
	СправочникиПрисоединенныхФайлов = СоставТипа(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Справочники, имена которых заканчиваются на ""%1""';
			|en = 'Catalogs whose names end on ""%1""'"), "ПрисоединенныеФайлы"), ОбъектыСФайлами);
	ВладельцыПрисоединенныхФайлов = СоставТипа(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Предусмотренный справочник присоединенных файлов с реквизитом %1 указанного типа';
			|en = 'The provided attachment catalog with attribute %1 of the specified type'"), "ВладелецФайлов"), МетаданныеВладельцев);
...
Процедура Подключаемый_РассылкаОтчетов_ПроверитьВнедрение()
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для выполнения рассылки отчетов выполните внедрение подсистемы ""%1"".';
				|en = 'To enable report distribution, integrate the ""%1"" subsystem.'"), "КонтактнаяИнформация");
	...
	Для Каждого ТипПолучателей Из ТипыПолучателей[0].Состав Цикл
		...
		Если КонтактнаяИнформацияГруппаСсылка = Неопределено Тогда
			...
			ДобавитьОшибку(ТипПолучателей, НСтр("ru = 'Отсутствует группа контактной информации';
												|en = 'Contact information group is missing'"), ТекстОшибки);
			Продолжить;
		КонецЕсли;
	...
	Для Каждого ТипПолучателей Из ТипыПолучателейСГруппамиКонтактнойИнформации Цикл
		...
		Если НЕ Выборка.НайтиСледующий(ПараметрыПоиска) Тогда
			...
			ДобавитьОшибку(ТипПолучателей.Ключ, НСтр("ru = 'Отсутствует вид контактной информации';
													|en = 'Contact information kind is missing'"), ТекстОшибки);
		КонецЕсли;
	КонецЦикла;
...
Процедура Подключаемый_Свойства_ПроверитьВнедрение()
		...
		Для Каждого ОбъектМетаданных Из ВидМетаданных Цикл
			...
			Если ТабличнаяЧастьДополнительныеРеквизиты <> Неопределено
				...
				Если Не ХарактеристикиЗаданы Тогда
					...
					ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Некорректно заполнены характеристики';
															|en = 'Characteristics are filled in incorrectly'"), ТекстОшибки);
				ИначеЕсли Не ХарактеристикиЗаданыКорректно И НаборОписанныйВКоде <> Неопределено Тогда
					...
					ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Некорректно заполнены поля отборов видов характеристики';
															|en = 'Fields of the characteristic type filters are filled in incorrectly'"), ТекстОшибки);
				ИначеЕсли Не ХарактеристикиЗаданыКорректно И НаборОписанныйВКоде = Неопределено Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У объекта ""%1"" с табличной частью ""%2""
		...
		Если ТипЗнч(ПредопределенныйНабор.Ключ) = Тип("Строка") Тогда
			Если ИменаПредопределенных.Найти(ПредопределенныйНабор.Ключ) <> Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Набор свойств ""%1"", описанный в процедуре %2
					...
					|is not marked with the Obsolete prefix in predefined items of catalog %3.'");
				...
				ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Дублирование предопределенных наборов свойств';
														|en = 'Duplicating predefined property sets'"), ТекстОшибки);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
	...
	ОбъектыСРеквизитамиИСведениями.Добавить(СоставТипа(НСтр("ru = 'Объекты с дополнительными реквизитами';
															|en = 'Objects with additional attributes'"), ОбъектыСДополнительнымиРеквизитами));
	...
	МассивИсточников.Добавить(СоставТипа(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Предопределенные элементы справочника %1';
				|en = 'Predefined items of catalog %1'"), "НаборыДополнительныхРеквизитовИСведений"),
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		Для Каждого ТипКритерия Из СоставКритерия Цикл
			Если СоставКоманды.Найти(ТипКритерия) = Неопределено Тогда
				...
				ДобавитьОшибку(ТипКритерия, НСтр("ru = 'Различается состав типов';
												|en = 'Composition of types differs'"), ПодробноеОписаниеОшибки);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура Подключаемый_ЭлектроннаяПодпись_ПроверитьВнедрение()
	...
	ТипПодписываемыеОбъекты = СоставТипа(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Объекты метаданных, содержащие реквизит %1';
			|en = 'Metadata objects that contain attribute %1'"), "ПодписанЭП"),
...
Процедура ДобавитьОшибку(Знач ОбъектМетаданных, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки,
	...
	ПодсистемаВерхнегоУровня = ?(ПодсистемыОбъекта = Неопределено, НСтр("ru = 'Без подсистемы';
																		|en = 'Without subsystem'"), ПодсистемыОбъекта[0].Представление);
...
Функция СоставПодписокПоОбработчику(Обработчик, ДопустимыеТипы = "", ИсключаемыеТипы = "")
	...
	Иначе
		...
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Подписки с обработчиком %1';
																				|en = 'Subscriptions with handler %1'"), ИмяОбработчика);
	КонецЕсли;
		...
		Для Каждого СтрокаТипа Из ТаблицаТипов Цикл
			Для Каждого Тип Из СтрокаТипа.Состав Цикл
				...
				Если Тип = Неопределено Тогда
					...
					КонецПопытки;
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В состав %1 включено значение Неопределено.';
							|en = 'Undefined value is included in the composition %1.'"), СтрокаТипа.Описание);
					ДобавитьОшибку(МетаданныеТипа, НСтр("ru = 'Значение Неопределено в составе типа';
														|en = 'Undefined value in the type composition'"), ТекстОшибки);
					Продолжить;
				КонецЕсли;
				...
				Если Отсутствующие.Количество() <> 0 Тогда
					...
					Если ПустаяСтрока(ОписаниеТипа) Тогда
						ОписаниеПроверяемогоТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Тип %1';
								|en = 'Type %1'"), ПолноеИмяТипа);
					Иначе
						ОписаниеПроверяемогоТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1 %2';
								|en = '%1 %2'"), ОписаниеТипа, ПолноеИмяТипа);
					КонецЕсли;	
					...
					ДобавитьОшибку(Тип, НСтр("ru = 'Различается состав типов';
											|en = 'Composition of types differs'"), ТекстОшибки);
				КонецЕсли;
		...
		Для Каждого СтрокаТипа Из ТаблицаТипов Цикл
			Для Каждого Тип Из СтрокаТипа.Состав Цикл
				...
				Если ЕстьОшибка Тогда
					...
					ДобавитьОшибку(Тип, НСтр("ru = 'Пересекается состав типов';
											|en = 'Composition of types overlaps'"), ТекстОшибки);
				КонецЕсли;
				ОбработанныеТипы.Вставить(Тип, Истина);
			КонецЦикла;
		КонецЦикла;
...
Процедура ПроверитьНаличиеПредопределенногоЭлемента(ПолноеИмяОбъектаМетаданных, ИмяПредопределенныхДанных)
	...
	Если МассивПредопределенных.Найти(ИмяПредопределенныхДанных) = Неопределено Тогда
		...
		ДобавитьОшибку(МетаданныеОбъекта, НСтр("ru = 'Отсутствует предопределенный элемент';
												|en = 'Predefined item is missing'"), ТекстОшибки);
	КонецЕсли;
	...
	Для Каждого СтрокаОбразца Из ТаблицаОбразец Цикл
		...
		Если ПроверяемыйРеквизит = Неопределено Тогда
			...
			Если НаличиеОбязательно Тогда
				...
				ДобавитьОшибку(МетаданныеСправочника, НСтр("ru = 'Отсутствует обязательный реквизит';
															|en = 'The required attribute is missing'"), ТекстОшибки);
			КонецЕсли;
		...
		Если Не ПустаяСтрока(ТипРеквизитаОбразец)
			...
			ДобавитьОшибку(МетаданныеСправочника, НСтр("ru = 'Несоответствие типов';
														|en = 'Type mismatch'"), ТекстОшибки);
		...
		Для Индекс = 3 По КоличествоКолонокВОбразце - 1 Цикл
			...
			Если Не СравнитьЗначениеСвойстваРеквизитаСОбразцом(ИмяКолонки, ЗначениеЯчейки, ЗначениеСвойства) Тогда
				...
				ДобавитьОшибку(МетаданныеСправочника, НСтр("ru = 'Неверное значение свойства';
															|en = 'Incorrect property value'"), ТекстОшибки);
			...
			Если Не ЭтоПоле Тогда // Это команда.
				Если (ПропускаемыйОбъект = Неопределено Или Не ПропускаемыйОбъект["View"])
					...
					ТекстОшибки = ?(Исправлять, НСтр("ru = 'Исправлено.';
													|en = 'Fixed.'") + " ", "")
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (лишнее право Просмотр)';
																						|en = '%1 (View right redundant)'"), ИмяРеквизита);
...
Процедура ПроверитьСоставОсновныхРолей()
	...
	Если Не ПропускатьНовыеПрава И ЭтоДемоБСП() И ИсправлятьОшибку("СоставОсновныхРолей") Тогда
		Для Каждого МакетыРоли Из МакетыОсновныхРолей Цикл
			...
			КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не обновлен эталонный макет для проверки роли %1.';
					|en = 'Reference template for checking the %1 role is not updated.'"), МакетыРоли.Ключ);
...
Процедура ПроверитьЦелостностьПодсистемыРаботаСФайлами()
	...
	Если ПодсистемаРаботаСФайлами = Неопределено Тогда
		Для Каждого МетаданныеСправочника Из Метаданные.Справочники Цикл
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка внедрения подсистемы ""Работа с файлами';
					|en = 'Cannot integrate the File management subsystem'"), ИмяСправочника);
			ДобавитьОшибку(МетаданныеСправочника,
				НСтр("ru = 'Существуют справочники, содержащие присоединенные файлы, при отсутствующей подсистеме ""Работа с файлами""';
					|en = 'Some catalogs contain attachments while the ""File management"" subsystem is missing'"), ТекстОшибки);
...
Процедура ВариантыОтчетов_ПроверитьПодключениеКФормамОтчета(ПараметрыПроверки, СтрокаОтчет)
	...
	Если ПодключенКОсновнойФорме <> ПодключенКФормеНастроек Тогда
		...
		ДобавитьОшибку(СтрокаОтчет.Метаданные, НСтр("ru = 'Отчет не подключен к общим формам';
													|en = 'Report is not attached to common forms.'"), ТекстОшибки);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВариантыОтчетов_ПроверитьПодключениеКХранилищуВариантов(ПараметрыПроверки, СтрокаОтчет)
	...
	ДобавитьОшибку(СтрокаОтчет.Метаданные, НСтр("ru = 'Отчет не подключен к хранилищу вариантов';
												|en = 'Report is not attached to the option storage'"), ТекстОшибки);
КонецПроцедуры
...
Процедура ВариантыОтчетов_ПроверитьНастройкиДляПоиска(ПредопределенныеВариантыОтчетов, ВариантОтчета)
	...
	ДобавитьОшибку(СведенияОбОтчете.Метаданные, НСтр("ru = 'Не заполнены описания вариантов отчетов';
													|en = 'Report option details are blank'"), ТекстОшибки);
КонецПроцедуры
...
Функция ДобавитьВидКИВТаблицуВидов(ПредопределенныйВид, ИмяПредопределенного, ТаблицаВидов)
	...
	Если Не ЭтоСправочник И Не ЭтоДокумент Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'The name of a predefined contact information kind must start
			|with either Catalog or Document. Current name: %1'"), ИмяПредопределенного);
...
Функция СоставОбъектовСРеквизитомДопУпорядочивания()
	...
	Возврат СоставТипа(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Объекты с реквизитом %1';
																	|en = 'Objects with attribute %1'"), "РеквизитДопУпорядочивания"),
		МассивМетаданных);
			...
			ДобавитьОшибку(МетаданныеПланаОбмена, НСтр("ru = 'Обращение к устаревшим настройкам плана обмена';
			    ...
			    СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В процедуре ""%1"" модуля менеджера плана обмена
						...
						|an attempt to access a non-existent setting is found:%2'"), "ПриПолученииНастроек", ПредставлениеОшибки));
			Продолжить;
		КонецПопытки;
				...
				ДобавитьОшибку(МетаданныеПланаОбмена, НСтр("ru = 'Обращение к устаревшим свойствам описания варианта настроек';
				    ...
				    СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В процедуре ""%1"" модуля менеджера плана обмена
							...
							|en = 'In procedure ""%1"" of exchange plan manager module,
							|an attempt to access a non-existent property is found:%2'"), "ПриПолученииОписанияВариантаНастройки", ПредставлениеОшибки));
			КонецПопытки;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПроверитьНаличиеМакетовИФорм()
	...
	Для Каждого ИмяПланаОбмена Из ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиПовтИсп").ПланыОбменаБСП() Цикл
		...
		Если ЕстьМакет Тогда
			Продолжить;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отсутствует макет ""%1""';
																						|en = 'Template ""%1"" is missing'"), "ПравилаРегистрации");
		КонецЕсли;
		
		ДобавитьОшибку(МетаданныеПланаОбмена, НСтр("ru = 'Отсутствуют правила регистрации';
													|en = 'Registration rules are missing'"), ТекстОшибки);
	...
	Для Каждого ПланОбмена Из Метаданные.ПланыОбмена Цикл
		...
		Если Не ПланОбмена.РаспределеннаяИнформационнаяБаза
			...
			Если Не ЗначениеЗаполнено(НастройкиОбмена.ИмяКонфигурацииПриемника)
				...
				ЗаголовокОшибки = НСтр("ru = 'Отсутствует обязательная настройка плана обмена';
										|en = 'Required exchange plan setting is missing'", ОбщегоНазначения.КодОсновногоЯзыка());
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В процедуре ""%1"" модуля менеджера плана обмена
					...
					|en = 'In procedure ""%1"" of exchange plan manager module,
					|value of required property ""%2"" is not specified'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьСоставОпределяемогоТипаКонечнаяТочкаОбменаСообщениями()
	...
	Если ИзбыточныеТипы.Количество() > 0 Тогда
		...
		ДобавитьОшибку(
			Метаданные.ОпределяемыеТипы["КонечнаяТочкаОбменаСообщениями"],
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Некорректный состав определяемого типа ""%1""';
																		|en = 'Incorrect composition of the %1 type collection:'"), "КонечнаяТочкаОбменаСообщениями"),
			ОписаниеОшибки);
	КонецЕсли;
...
Процедура ПроверитьВключатьРасширениеКонфигурацииДляПлановОбменаВМоделиСервиса()
	...
	Для Каждого ПланОбмена Из Метаданные.ПланыОбмена Цикл
		...
		Если ПланОбмена.РаспределеннаяИнформационнаяБаза
			...
			ЗаголовокОшибки = НСтр("ru = '""Включать расширение конфигурации"" в модели сервиса';
									|en = '""Attach configuration extension"" in SaaS mode'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			ОписаниеОшибки = НСтр("ru = 'Не допустимо использовать параметр ""Включать расширение конфигурации"" для распределенных планов обмена,
				...
				|used in SaaS mode'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроверитьСоставОбщихКоманд()
	...
	Для Каждого ИмяПланаОбмена Из ВсеПланыОбменаБСП Цикл
		...
		Если НедостающиеСостав.Количество() > 0 Тогда
			...
			ДобавитьОшибку(ПланОбмена, НСтр("ru = 'План обмена не включен в состав команд';
											|en = 'Exchange plan is not included in command composition'"), ТекстОшибки);
		КонецЕсли;
		...
		Если ИзбыточныеСостав.Количество() > 0 Тогда
			...
			ДобавитьОшибку(ПланОбмена, НСтр("ru = 'План обмена избыточно включен в состав команд';
											|en = 'Exchange plan is redundantly included in command composition'"), ТекстОшибки);
		КонецЕсли;
	...
	Для Каждого ПланОбмена Из ПланыОбменаПодсистемы Цикл
		...
		Для Каждого ОбъектПланаОбмена Из ПланОбмена.Состав Цикл
			...
			Если ОбъектПланаОбмена.АвтоРегистрация = АвтоРегистрацияИзменений.Разрешить Тогда
				...
				ДобавитьОшибку(ОбъектПланаОбмена.Метаданные, НСтр("ru = 'Некорректное использование признака авторегистрации';
																	|en = 'Autoregistration set incorrectly'"), ТекстОшибки);
			КонецЕсли;
		КонецЦикла;
...
Процедура ДобавитьОбъект(ИмяОбъекта, Исключения)
	...
	ИначеЕсли ЭтоДемоБСП() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В функции %1 указан несуществующий объект метаданных %2';
																					|en = 'Non-existent metadata object %2 is specified in function %1'"), "ОбъектыИсключенияПланаОбменаРИБ", ИмяОбъекта);
		ДобавитьОшибку(Метаданные,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Некорректно заполнены %1';
																		|en = '%1 are filled in incorrectly'"), "ОбъектыИсключенияПланаОбменаРИБ"),
			ТекстОшибки);
	КонецЕсли;
		...
		Для Каждого ДопустимыйТип Из ДопустимыеМетаданные Цикл
			Если ДопустимыйТип.Содержит(Объект) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Согласно документации, подсистема %1 не 
					...
					|contain objects to include in the exchange plan.'"), ИмяПодсистемы);
				ДобавитьОшибку(Объект, НСтр("ru = 'Подсистема содержит объекты с данными';
											|en = 'The subsystem contains objects with data'"), ТекстОшибки);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПроверитьСоставПланаОбмена(ИмяПланаОбмена, ПлановыйСостав, ФактическийСостав)
	...
	Для Каждого Объект Из НедостающиеОбъекты Цикл
		...
		ДобавитьОшибку(Объект, НСтр("ru = 'Объект должен быть включен в состав плана обмена';
									|en = 'Object must be included in exchange plan'"), ТекстОшибки);
	КонецЦикла;
	...
	Для Каждого Объект Из ИзбыточныеОбъекты Цикл
		...
		ДобавитьОшибку(Объект, НСтр("ru = 'Объект избыточно включен в состав плана обмена';
									|en = 'Object is excessively included in the exchange plan'"), ТекстОшибки);
	КонецЦикла;
		...
		Для Каждого СтрокаТаблицы Из ФактическийСостав Цикл
			...
			Если СтрокаТаблицы.Состав.Найти(Объект) = Неопределено Тогда
				...
				ДобавитьОшибку(Объект, 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект должен быть включен в подписку вида <%1><%2>';
							|en = 'Object must be included in subscription of the following kind: <%1><%2>'"), "ИмяПланаОбмена", "ВидПодписки"), ТекстОшибки);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого СтрокаТаблицы Из ФактическийСостав Цикл
		Для Каждого Объект Из СтрокаТаблицы.Состав Цикл
			Если ПлановыйСостав.Найти(Объект) = Неопределено Тогда
				...
				ДобавитьОшибку(Объект, 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект избыточно включен в подписку вида <%1><%2>';
							|en = 'Object is excessively included in subscription of the following kind: <%1><%2>'"), "ИмяПланаОбмена", "ВидПодписки"), ТекстОшибки);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция СоставПодписок(ИмяПланаОбмена)
	...
	Для Каждого СвойствоПодписки Из СвойстваПодписок Цикл
		...
		Описание = ?(КоличествоПодписок = 1, ТекущаяПодписка.ПолноеИмя(),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'подписки с именем %1';
																		|en = 'subscriptions with the %1 name'"), СвойствоПодписки.Ключ));
...
Процедура ПроверитьПараллельныйОтложенныйОбработчик(Знач Обработчик, Знач СоставПланаОбмена, ОбработчикиДляПроверки, ВыявленныеОшибки, ИмяПодсистемы)
	...
	Если ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмяОбъектаОбработчикаОбновления) = Неопределено Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Обработчик.Процедура, "Процедура")
			+ Символы.ПС
			+ НСтр("ru = '- не найден объект';
					|en = '- object is not found'") + " " + ПолноеИмяОбъектаОбработчикаОбновления;
	...
	Иначе
		...
		Для Каждого ЧитаемыйОбъект Из ЧитаемыеОбъекты Цикл
			...
			Если Не ОбъектНеСуществует Тогда
				...
				+ НСтр("ru = '- не найден объект';
						|en = '- object is not found'") + " " + ЧитаемыйОбъект;
			Иначе
	...
	Иначе
		...
		Для Каждого ИзменяемыйОбъект Из ИзменяемыеОбъекты Цикл
			...
			Если Не ОбъектНеСуществует Тогда
				НесуществующиеОбъекты = НесуществующиеОбъекты + ?(НесуществующиеОбъекты = "", "", Символы.ПС)
					+ НСтр("ru = '- несуществующий объект метаданных';
							|en = '- non-existing metadata object'") + " " + ИзменяемыйОбъект;
				Продолжить;
			КонецЕсли;
			...
			Если СтрокиВхождения.Количество() Тогда
				...
				Если МассивПараметровВызоваМетода.Количество() >= 3 Тогда
					...
					Если СтрНайти(ИмяШагаВВызове, НРег(ИмяШага)) > 0 Тогда
						...
						ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Для имени шага ключевой операции используется имя ""Удельное""';
																|en = 'Specific name is used for a step name of the key operation'"), ТекстКомментария);
					КонецЕсли;					
				КонецЕсли;				
			КонецЕсли;
	...
	Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
		...
		Для Каждого ЗапрещенныйПараметр Из ЗапрещенныеПараметры Цикл
			Если СтрНайтиНеКомментарийИНеСтроку(ТекстМодуля, ЗапрещенныйПараметр) <> 0 Тогда
				...
				КонецЕсли;
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В %1 обнаружено прямое обращение к %2.';
						|en = 'Direct access to %2 is detected in %1.'"), СвойстваОбъекта.Представление, ЗапрещенныйПараметр);
...
Процедура ПроверитьСоставРазделителей()
	...
	Для Каждого СвойстваРазделителя Из Разделители Цикл
		...
		Для Каждого СвойствоСоставаРеквизита Из СвойстваРазделителя.Значение Цикл
			Если МетаданныеРазделителя[СвойствоСоставаРеквизита.Ключ] <> СвойствоСоставаРеквизита.Значение Тогда
				...
				ДобавитьОшибку(МетаданныеРазделителя, НСтр("ru = 'Некорректное значение свойства разделителя';
															|en = 'Incorrect separator property value'"), ТекстОшибки);
	...
	Для Каждого СвойстваРазделителя Из Разделители Цикл
				...
				ИначеЕсли Не ПропуститьОшибку Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						МетаданныеРазделителя.Имя, ?(ПлановоеЗначениеОтличаетсяОтАвто, СвойстваРазделителя.Значение, НСтр("ru = 'Авто';
																															|en = 'Auto'")));
					ДобавитьОшибку(МетаданныеОбъекта, НСтр("ru = 'Некорректное значение разделителя';
															|en = 'Incorrect separator value'"), ТекстОшибки);
					ЕстьОшибкиСоставаРазделителей = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Если ИсправлятьОшибку("НекорректныйСоставРазделителей") И ВнесеныИзменения И Не ЕстьПропущенныеОшибки Тогда
			...
			ДобавитьОшибку(МетаданныеРазделителя, НСтр("ru = 'Некорректное значение состава разделителя';
														|en = 'Incorrect separator content value'"), ТекстОшибки);
		КонецЕсли;
	КонецЦикла;
	...
	Если ИсправлятьОшибку("ИзбыточнаяУстановкаРазделителя") Тогда
		Для Каждого СвойстваРазделителя Из Разделители Цикл
			...
			Если УдаляемыеУзлы.Количество() > 0 Тогда
				...
				ДобавитьОшибку(МетаданныеРазделителя, НСтр("ru = 'Избыточное использование явного значения разделителя';
															|en = 'Redundant use of explicit separator value'"), ТекстОшибки);
			КонецЕсли;
		КонецЦикла;
	Иначе
		...
		Для Каждого СвойстваРазделителя Из Разделители Цикл
			...
			Для Каждого ЭлементСостава Из МетаданныеРазделителя.Состав Цикл
				Если ЭлементСостава.Использование = СвойстваРазделителя.Значение Тогда
					...
					ДобавитьОшибку(ЭлементСостава.Метаданные, НСтр("ru = 'Избыточное использование явного значения разделителя';
																	|en = 'Redundant use of explicit separator value'"), ТекстОшибки);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
...
Процедура СоставРолиАдминистраторСистемы(РазделенныеОбъектыМетаданных, НеразделенныеОбъектыМетаданных, Исключения)
	...
	Пока Истина Цикл
		...
		Если ЧастиИмени.Количество() > 2 Тогда
			...
			Иначе
				...
				ДобавитьОшибку(ОбъектМетаданных,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В роли %1 должны быть всегда установлены права на реквизиты';
																				|en = 'Access rights to attributes must always be set in role %1'"), "АдминистраторСистемы"),
					ТекстОшибки);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
			...
			Иначе
				Если ЭтоДемоБСП() Тогда
					ШаблонОшибки = НСтр("ru = 'В роли %1 избыточно установлены права на %2.
						...
						|The separator values may be incorrect.'");
				Иначе
					ШаблонОшибки = НСтр("ru = 'В роли %1 избыточно установлены права на %2';
										|en = 'Redundant access rights to %2 are set in role %1'");
				КонецЕсли;
				...
				ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяДляОшибки);
				ДобавитьОшибку(ОбъектМетаданных,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В роли %1 избыточно установлены права на разделенный объект';
																				|en = 'Redundant access rights to a separated object are set in role %1'"), "АдминистраторСистемы"),
					ТекстОшибки);
			КонецЕсли;
		ИначеЕсли НеразделенныеОбъектыМетаданных.Получить(ПолноеИмяОбъекта) = Ложь Тогда
			...
			Если ЗначениеЗаполнено(СписокПрав) Тогда
				...
				ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяДляОшибки);
				ДобавитьОшибку(ОбъектМетаданных,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В роли %1 установлены запрещенные права на объект';
																				|en = 'Restricted access rights to an object are set in role %1'"), "АдминистраторСистемы"),
					ТекстОшибки);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СписокПрав) Тогда
				...
				ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяДляОшибки);
				ДобавитьОшибку(ОбъектМетаданных,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В роли %1 не установлены необходимые права на объект';
																				|en = 'Required access rights to an object are not set in role %1'"), "АдминистраторСистемы"),
					ТекстОшибки);
			КонецЕсли;
	...
	Если ВнесеныИзменения Тогда
		...
		ДобавитьОшибку(РольАдминистраторСистемы, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Права для роли %1 не соответствуют ожидаемым';
																		|en = 'Access rights for role %1 do not match the expected rights'"), "АдминистраторСистемы"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Исправлено. Права для роли %1 установлены согласно значению разделителей';
																		|en = 'Fixed. Access rights for role %1 are set according to separator values'"), "АдминистраторСистемы"));
...
Процедура СоставРолиПолныеПрава(РазделенныеОбъектыМетаданных, НеразделенныеОбъектыМетаданных, Исключения)
	...
	Пока Истина Цикл
		...
		Если ЧастиИмени.Количество() > 2 Тогда
			...
			Иначе
				...
				ДобавитьОшибку(ОбъектМетаданных,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В роли %1 должны быть всегда установлены права на реквизиты';
																				|en = 'Access rights to attributes must always be set in role %1'"), "ПолныеПрава"),
					ТекстОшибки);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
			...
			Если ЗначениеЗаполнено(СписокПрав) Тогда
				Если ЭтоДемоБСП() Тогда
					ШаблонОшибки = НСтр("ru = 'В роли %1 для %2 установлены запрещенные права:
						|%3.
						|Возможно некорректно установлены значения разделителей.';
						|en = 'The restricted access rights are set for %2 in role %1:
						|%3.
						|The separator values may be incorrect.'");
				Иначе
					ШаблонОшибки = НСтр("ru = 'В роли %1 для %2 установлены запрещенные права:
				...
				ДобавитьОшибку(ОбъектМетаданных,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В роли %1 установлены запрещенные права на объект';
																				|en = 'Restricted access rights to an object are set in role %1'"), "ПолныеПрава"),
					ТекстОшибки);
			КонецЕсли;
		...
		ИначеЕсли РазделенныеОбъектыМетаданных.Получить(ПолноеИмяОбъекта) = Ложь Тогда
			...
			Если ЗначениеЗаполнено(СписокПрав) Тогда
				...
				ДобавитьОшибку(ОбъектМетаданных,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В роли %1 не установлены необходимые права на объект';
																				|en = 'Required access rights to an object are not set in role %1'"), "ПолныеПрава"),
					ТекстОшибки);
			КонецЕсли;
	...
	Если ВнесеныИзменения Тогда
		...
		ДобавитьОшибку(РольПолныеПрава,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Права для роли %1 не соответствуют ожидаемым';
																		|en = 'Access rights for role %1 do not match the expected rights'"), "ПолныеПрава"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Исправлено. Права для роли %1 установлены согласно значению разделителей';
																		|en = 'Fixed. Access rights for role %1 are set according to separator values'"), "ПолныеПрава"));
...
Функция БазовыеСвойстваРолиУстановленыКорректно(ДокументDOM, МетаданныеРоли, УстанавливатьПраваДляНовыхОбъектов, Разыменователь = Неопределено)
	...
	БазовыеСвойства.Добавить(СвойствоУстанавливатьПраваДляНовыхОбъектов(), НСтр("ru = 'Устанавливать права для новых объектов';
																				|en = 'Set rights for new objects'"), УстанавливатьПраваДляНовыхОбъектов);
	БазовыеСвойства.Добавить(СвойствоУстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию(), НСтр("ru = 'Устанавливать права для реквизитов и табличных частей по умолчанию';
																										|en = 'Set rights for attributes and tables by default'"), Истина);
	БазовыеСвойства.Добавить(СвойствоНезависимыеПраваПодчиненныхОбъектов(), НСтр("ru = 'Независимые права подчиненных объектов';
																				|en = 'Independent rights of subordinate objects'"), Ложь);
	...
	Для Каждого БазовоеСвойство Из БазовыеСвойства Цикл
		...
		Если ЗначениеСвойства <> БазовоеСвойство.Пометка Тогда
			...
			ДобавитьОшибку(МетаданныеРоли,
				НСтр("ru = 'Значение базового свойства роли отличается от поставляемого';
					|en = 'The role basic property value does not match the built-in property value.'"), ТекстОшибки);
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОпределитьВидыОграниченийПрав(Параметры)
	...
	Для каждого Строка Из Ограничения Цикл
		Если СтрСравнить(Строка.Поля, СтрокаПрочиеПоля) <> 0 Тогда
			...
			ДобавитьОшибку(Роль, НСтр("ru = 'Ограничение не по всем полям';
										|en = 'The restriction is not by all fields'"), ТекстОшибки);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьИмяТаблицы(Свойства)
	...
	Если Свойства.Таблица <> Свойства.УказаннаяТаблица Тогда
		...
		ДобавитьОшибку(Роль, НСтр("ru = 'Неверное имя таблицы';
									|en = 'Incorrect table name'"), ТекстОшибки);
	КонецЕсли;
...
Процедура ПроверитьИмяПрава(Свойства)
	...
	Если ЗначениеЗаполнено(Свойства.УказанноеПраво)
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Свойства.Роль, Свойства.Поля, Свойства.Право, Свойства.Таблица, Свойства.Ограничение);
		Роль = Метаданные.Роли.Найти(Свойства.Роль);
		ДобавитьОшибку(Роль, НСтр("ru = 'Неверное имя права';
									|en = 'Incorrect right name'"), ТекстОшибки);
	КонецЕсли;
...
Процедура ДобавитьВидДоступа(Знач Свойства, Знач ВидыОграниченийПрав,
	...
	Для Каждого ВидДоступа Из ВидыДоступа Цикл
		...
		Если МодульУправлениеДоступомСлужебный.СвойстваВидаДоступа(ВидДоступа) = Неопределено Тогда
			...
			Роль = Метаданные.Роли.Найти(Свойства.Роль);
			ДобавитьОшибку(Роль, НСтр("ru = 'Несуществующий вид доступа';
										|en = 'Non-existent access kind'"), ТекстОшибки);
			Продолжить;
		КонецЕсли;
...
Процедура ДобавитьОграниченияПравРоли(ОграниченияПрав, Роль)
	...
	Для Каждого ОписаниеОбъекта Из Объекты Цикл
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти объект метаданных ""%1"".';
					|en = 'Cannot find the ""%1"" metadata object.'"), ОписаниеОбъекта.Ключ);
		...
		Для Каждого ОписаниеПрава Из СоответствиеПрав Цикл
			...
			Для Каждого ОписаниеОграничения Из ОграниченияПолей Цикл
				Если ОписаниеОграничения.Ключ = "" И Не ЗначениеЗаполнено(ОписаниеОграничения.Значение) Тогда
					Продолжить;
				КонецЕсли;
				Поля = ?(ОписаниеОграничения.Ключ = "", НСтр("ru = '<Прочие поля>';
															|en = '<Other fields>'"), ОписаниеОграничения.Ключ);
...
Процедура ДобавитьПраваРоли(Роль, Контекст)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'При чтении прав роли %1 произошла ошибка:';
			|en = 'An error occurred when reading the %1 role rights:'"), Роль);
	...
	Для Каждого ОпределяемыйТип Из СоставОпределяемыхТипов Цикл
		...
		Если ИсправлятьОшибку("ТипОтсутствуетВСоставеОпределяемогоТипа") И СоставТипов.Количество() > 0 Тогда
			...
			Если ТипИОМ <> Неопределено Тогда
				...
				ЕстьИзменения = Истина;
				ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип %1 присутствует в определяемом типе.';
						|en = 'The %1 type is present in the type collection.'"), "СправочникОбъект.ИдентификаторыОбъектовМетаданных");
				ДобавитьОшибку(ОбъектМетаданных, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Исправлено. Тип %1 исключен из состава определяемого типа.';
						|en = 'Fixed. The %1 type is excluded from the type collection.'"), "СправочникОбъект.ИдентификаторыОбъектовМетаданных"));
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьТипыИзмеренийРегистра(ДокументDOM, Знач ОбъектМетаданных, Знач ОписаниеПолей)
	...
	Для Индекс = 1 По СписокПолей.Количество() Цикл
		...
		Если СоставТипов = Неопределено Тогда
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отсутствует поле ""%1"" в регистре ""%2""';
				|en = 'The %1 field in the %2 register is missing'"), ИмяПоля, ПредставлениеРегистра);
			Продолжить;
		КонецЕсли;
...
Процедура ПроверитьВставкуПроцедурыПриЗаполненииОграниченияДоступаВМодулеМенеджераОбъекта(Параметры, ОбъектМетаданных)
	...
	Если ПроцедураМодуля = Неопределено Тогда
		ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Отсутствует обязательная вставка кода';
												|en = 'The required code insert is missing'"), 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В модуле менеджера должна быть процедура %1.';
					|en = 'Manager module must contain procedure %1.'"), "ПриЗаполненииОграниченияДоступа"));
	КонецЕсли;
					...
					Если РазныйТекстОграниченияДляПользователейИВнешнихПользователейВОднойРоли Тогда
						ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Исправлено частично для таблицы %1.
							           ...
							           |
							           |2. User restriction texts for table %1 are refreshed.'") + Символы.ПС + Символы.ПС + ПояснениеПоТекстам,
							ОбъектМетаданных.ПолноеИмя(), НСтр("ru = 'Было:';
																|en = 'Previous value:'"), НСтр("ru = 'Стало:';
																					|en = 'Current value:'"), Текст, ТекстДляВнешних);
					Иначе
						ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Исправлено. Обновлены тексты ограничений для таблицы %1.';
								|en = 'Fixed. Restriction texts for the %1 table are updated.'") + Символы.ПС + Символы.ПС + ПояснениеПоТекстам,
							ОбъектМетаданных.ПолноеИмя(), НСтр("ru = 'Было:';
																|en = 'Previous value:'"), НСтр("ru = 'Стало:';
																					|en = 'Current value:'"));
					КонецЕсли;
					...
					Если РазныйТекстОграниченияДляПользователейИВнешнихПользователейВОднойРоли Тогда
						ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '1. Для таблицы %1 невозможно обновить тексты ограничений
							           ...
							           |or make separate roles for users and external users.
							           |
							           |2. Refresh user restriction texts for the %1 table.'") + Символы.ПС + Символы.ПС + ПояснениеПоТекстам,
							ОбъектМетаданных.ПолноеИмя(), НСтр("ru = 'Текущее значение:';
																|en = 'Current value:'"), НСтр("ru = 'Заменить для пользователей на:';
																								|en = 'Replace for users with:'"), Текст, ТекстДляВнешних);
					Иначе
						ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Обновите тексты ограничений для таблицы %1';
								|en = 'Update restriction texts for the %1 table'") + Символы.ПС + Символы.ПС + ПояснениеПоТекстам,
							ОбъектМетаданных.ПолноеИмя(), НСтр("ru = 'Текущее значение:';
																|en = 'Current value:'"), НСтр("ru = 'Заменить на:';
																								|en = 'Replace with:'"));
					КонецЕсли;
				КонецЕсли;
...
Процедура ПроверитьИспользованиеНовогоRLSВРолях(ОграниченияДоступа, СпискиСОграничениемДоступа)
	...
	Для Каждого Таблица Из ТаблицыБезНовогоRLS Цикл
		...
		Если СписокРолей.Количество() = 1 Тогда
			ТекстПояснения = НСтр("ru = 'Доступ к объекту на уровне записей ограничен в роли';
									|en = 'Access to object on the record level is restricted in role'") + " " + СписокРолей[0];
		Иначе
			ТекстПояснения = НСтр("ru = 'Доступ к объекту на уровне записей ограничен в ролях:';
									|en = 'Access to object on the record level is restricted in roles:'") + Символы.ПС 
		...
		КраткоеОписаниеОшибки = НСтр("ru = 'Отсутствует обязательная вставка кода';
									|en = 'The required code insert is missing'");
		ПодробноеОписаниеОшибки = НСтр("ru = 'В процедуре %1 отсутствует обязательная вставка кода:';
										|en = 'Procedure %1 does not contain the required code insert:'") + Символы.ПС
...
Процедура ПроверитьНаличиеШаблонаВРоли(ДокументDOM, Роль, ИмяШаблона, ПоставляемыйШаблон, ЕстьИзменения)
	...
	Если Не ШаблонИспользуется Тогда
		...
		КонецЕсли;
		ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Шаблон #%1 не используется в роли.';
				|en = 'Template #%1 is not used in the role.'"), ИмяШаблона);
		Если ИсправлятьОшибку("ШаблонНеИспользуетсяВРоли") Тогда
			...
			ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Исправлено. Неиспользуемый шаблон #%1 удален.';
					|en = 'Fixed. Not used template #%1 has been deleted.'"), ИмяШаблона));
		Иначе
			ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Шаблон #%1 не используется, удалите его.';
					|en = 'Template #%1 is not used, delete it.'"), ИмяШаблона));
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НайденныйШаблон <> Неопределено Тогда
		...
		Если ТекстИмеющегосяШаблона <> ТекстПоставляемогоШаблона Тогда
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Шаблон #%1 отличается от поставляемого.';
					|en = 'Template #%1 differs from the built-in template.'"), ИмяШаблона);
			Если ИсправлятьОшибку("ИспользуемыйШаблонВРолиОтсутствуетИлиОтличается") Тогда
				...
				ЕстьИзменения = Истина;
				ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Исправлено. Шаблон #%1 обновлен из поставки конфигурации.';
						|en = 'Fixed. Template #%1 is updated from the configuration package.'"), ИмяШаблона));
			Иначе
				ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Текст шаблона #%1 скопируйте из роли %2.';
						|en = 'Copy the text of template #%1 from role %2.'"), ИмяШаблона, "ИзменениеУчастниковГруппДоступа"));
			КонецЕсли;
		КонецЕсли;
	Иначе
		...
		Если ИсправлятьОшибку("ИспользуемыйШаблонВРолиОтсутствуетИлиОтличается") Тогда
			...
			ЕстьИзменения = Истина;
			ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Исправлено. В роль добавлен шаблон #%1';
					|en = 'Fixed. Template #%1 is added to the role'"), ИмяШаблона));
		Иначе
			ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В роль скопируйте шаблон #%1 из роли %2.';
					|en = 'Copy template #%1 to the role from role %2.'"), ИмяШаблона, "ИзменениеУчастниковГруппДоступа"));
		КонецЕсли;
	КонецЕсли;
...
Функция ВидыБлоков()
	...
	Результат.Вставить(КлючевоеСловоОбласть(), "#КонецОбласти");
...
Процедура ПрочитатьСтруктуруМодуля(Модуль)
	Если Не ЗначениеЗаполнено(Модуль.Структура) Тогда
		Модуль.Структура = СтрокаВБлок(Модуль.ТекстМодуля);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьПредопределенные(ОбъектыСДополнительнымиСвойствами, ИмяПредопределенного, ДопустимыеПрефиксы)
	...
	Если КоллекцияМетаданных = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Имя предопределенного набора свойств должно начинаться
			...
			| with the description of reference type (""Catalog"", ""Document"" and so on). Current name is ""%1""'"), ИмяПредопределенного);
		ДобавитьОшибку(МетаданныеНаборов, НСтр("ru = 'Некорректное имя предопределенного набора свойств';
												|en = 'Incorrect name of the predefined property set'"), ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если МетаданныеВладельца = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для предопределенного набора свойств %1 отсутствует объект метаданных %2';
			...
			ИмяПредопределенного, ИмяВладельца);
		ДобавитьОшибку(МетаданныеНаборов, НСтр("ru = 'Отсутствует объект метаданных';
												|en = 'Metadata object is missing'"), ТекстОшибки);
	Иначе
...
Процедура ВыгрузитьКонфигурациюВXML(КаталогВыгрузкиКонфигурации)
	...
	Если Не ПустаяСтрока(КаталогВыгрузкиКонфигурации) Тогда
		...
		Если Не Каталог.Существует() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Указанный каталог выгрузки ""%1"" не существует.';
																							|en = 'Specified ""%1"" export directory does not exist.'"), КаталогВыгрузкиКонфигурации);
		КонецЕсли;
	...
	Если Результат.КодВозврата <> 0 Тогда
		...
		Попытка
			...
			Если Сообщения.Количество() = 0 Тогда
				...
				НСтр("ru = 'Код возврата запуск системы 1С:Предприятие в режиме Конфигуратора: %1';
					|en = 'Return code at 1C:Enterprise startup in Designer mode: %1'"), Результат.КодВозврата));
			КонецЕсли;
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка внедрения БСП';
										|en = 'SSL integration check'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			Если ПустаяСтрока(Сообщения) Тогда
				Сообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Код возврата: %1';
						|en = 'Return code: %1'"), Результат.КодВозврата);
			КонецЕсли;
		Исключение
			Сообщения = "";
		КонецПопытки;
...
Процедура ТекстМодуляСодержитПроцедуру(ОбъектМетаданных, ТекстМодуля, Знач ТипМодуля, СтрокаКода, ИмяПроцедурыИлиФункции, ДеталиПроверки)
	...
	Если ТипЗнч(СтрокаКода) = Тип("Массив") Тогда
		СтрокаВызова = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" или ""%2""';
																					|en = '%1 or %2'"), СтрокаКода[0], СтрокаКода[1]);
	Иначе
		СтрокаВызова = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1""';
																					|en = '""%1""'"), СтрокаКода);
	КонецЕсли;
...
Процедура ПроверитьИсходныйКодОбъектаМетаданныхПоОбразцу(ОбъектМетаданных, ПроверяемыеВставки, ПроверяемыеМодули)
	...
	Для Каждого ПроверяемыйМодуль Из ПроверяемыеМодули Цикл
		...
		Если ПустаяСтрока(ТекстМодуля) Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отсутствует текст модуля: %1.';
					|en = 'Module text is missing: %1.'"), ПроверяемыйМодуль);
		...
		Для Каждого ВставкаКода Из ПроверяемыеВставки Цикл
			...
			Если Не ПустаяСтрока(ТекстПроверяемойПроцедуры) Тогда
				...
				Иначе
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Отсутствует вызов: %1.';
							|en = 'Call is missing: %1.'"), ВставкаКода.Размещение);
		...
		Если СтруктураОбъекта.Использование Тогда
			Если Не СтруктураОбъекта.Корректность Тогда
				...
				ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Модуль формы %1';
																								|en = '%1 form module'"), ПроверяемыйМодуль);
...
Функция СвойствоРоли(ТекстОшибки, КраткоеОписаниеОшибки, Исправлять, ДокументDOM, ИмяСвойства, ТребуемоеЗначение = Неопределено)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Неизвестное свойство роли';
							|en = 'Unknown role property'") + ": " + ИмяСвойства;
	...
	Если Значение = Неопределено Тогда
		ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Свойство ""%1"" не найдено.';
				|en = 'The %1 property is not found.'"), ПредставлениеСвойства);
	...
	Если ЭтоДемоБСП() Тогда
		Если ТипЗнч(Значение) <> Тип("Булево") Тогда
			...
			Иначе
				ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Свойство ""%1"" не типа Булево.';
						|en = 'The %1 property is not Boolean.'"), ПредставлениеСвойства);
		...
		ИначеЕсли ТипЗнч(ТребуемоеЗначение) = Тип("Булево")
			...
			ИначеЕсли ТребуемоеЗначение Тогда
				ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Свойство ""%1"" должно быть установлено.';
						|en = 'The %1 property must be set.'"), ПредставлениеСвойства);
			...
			Иначе
				ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Свойство ""%1"" должно быть снято.';
						|en = 'The %1 property must be cleared.'"), ПредставлениеСвойства);
...
Процедура ЗадатьКаталогВыгрузкиКонфигурации(КаталогВыгрузкиКонфигурации)
	...
	Если НайтиФайлы(КаталогВыгрузкиКонфигурации, "Configuration.xml").Количество() = 0 Тогда
		Если НайтиФайлы(КаталогВыгрузкиКонфигурации, ".project").Количество() = 0 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Указанный каталог выгрузки ""%1"" не содержит файлов выгрузки конфигурации.';
																							|en = 'Specified ""%1"" export directory does not contain configuration export files.'"), КаталогВыгрузкиКонфигурации);
		Иначе
	...
	Для НомерСтроки = НомерСтрокиРазделаОписанияПеременных + 1 По КоличествоСтрок Цикл
		...
		Если СтрНачинаетсяС(ТекущаяСтрока, "#КонецОбласти") Тогда
			Если НЕ ПеременнаяПараметрыПриложенияОпределена Тогда
				КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не определена обязательная переменная %1.';
						|en = 'The required %1 variable is not determined.'"), "ПараметрыПриложения");
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция РезультатПроверкиПодсистемы(ИмяМетода, ПодсистемаБСП, ХранилищеСтруктурыИнициализации, Проверка = Неопределено) Экспорт
	...
	Исключение
		ДобавитьОшибку(ПроверяемаяПодсистема, НСтр("ru = 'Проверка внедрения подсистемы не выполнена:';
													|en = 'Subsystem integration is not checked:'"), ИнформацияОбОшибке());
	КонецПопытки;
	...
	Для Каждого АдресРезультатаПроверки Из АдресаРезультатовПроверок Цикл
		...
		Иначе
			...
			Если ТипЗнч(ТаблицаРезультатаПроверки) <> Тип("Структура") Тогда
				ДобавитьОшибку(Неопределено, АдресРезультатаПроверки.Ключ,
					НСтр("ru = 'Нет результата проверки.';
						|en = 'There is no check result.'"),, Истина);
				Продолжить;
			КонецЕсли;
		КонецЕсли;
