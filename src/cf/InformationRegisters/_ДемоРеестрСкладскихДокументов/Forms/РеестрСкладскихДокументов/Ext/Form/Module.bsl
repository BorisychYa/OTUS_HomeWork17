...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Идентификатор Из ИдентификаторыТипов Цикл
		...
		Если Права.Добавление Тогда
			...
			Команда = Команды.Добавить(ИмяКомандыСоздать() + ОбъектМетаданных.Имя);
			Команда.Действие = "Подключаемый_Создать";
			Команда.Заголовок = ОбъектМетаданных.Представление();
			Кнопка = Элементы.Добавить(ИмяКомандыСоздать() + ОбъектМетаданных.Имя, Тип("КнопкаФормы"), Элементы.Создать);
...
Процедура Подключаемый_Создать(Команда)
	...
	ИмяФормыДокумента = "Документ." + Сред(Команда.Имя, СтрДлина(ИмяКомандыСоздать() +1)) + ".ФормаОбъекта";
...
Процедура ПометитьНаУдалениеСнятьПометкуЗавершение(Ответ, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(ТекстОповещения,
		ПолучитьНавигационнуюСсылку(Контекст.Ссылка), Строка(Контекст.Ссылка), БиблиотекаКартинок.ДиалогИнформация);
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неизвестный тип документа %1';
				|en = 'Unknown document type %1'"), ТипЗнч(Ссылка));
	КонецЕсли;
...
Процедура УстановитьПроводки(Провести)
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение';
										|en = 'Update'"),
		ПолучитьНавигационнуюСсылку(ТекущиеДанные.Ссылка), Строка(ТекущиеДанные.Ссылка), БиблиотекаКартинок.ДиалогИнформация);
