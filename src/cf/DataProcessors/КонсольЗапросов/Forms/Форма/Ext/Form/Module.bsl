...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru = 'Консоль запросов (%ИмяЗапросаПоУмолчанию%)';
							|en = 'Query console (%ИмяЗапросаПоУмолчанию%)'");
...
Процедура ЗаполнитьИзXML(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		СообщитьПользователю(НСтр("ru = 'Выберите запрос.';
									|en = 'Select query.'"), "Объект");
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрЗапросы Из Объект.Запросы Цикл
		...
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса Тогда
			...
			РезультирующаяСтрока	= НСтр("ru = 'Результат запроса (количество строк = %КолСтрок%, время выполнения = %ВремяВыполнения% с)';
											|en = 'Query result (number of rows = %КолСтрок%, execution time = %ВремяВыполнения% sec)'");
...
Процедура СохранитьЗапросы()
	...
	Если Не ПустаяСтрока(ИмяФайла) Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Автосохранение прошло успешно.';
											|en = 'Successfully autosaved.'"), ИмяФайла);
		Модифицированность = Ложь;
	КонецЕсли;
...
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения)
	...
	РезультирующаяСтрока = НСтр("ru = 'Результат запроса (количество строк = %КоличествоСтрок%, время выполнения = %ВремяВыполнения% с)';
								|en = 'Query result (number of rows = %КоличествоСтрок%, execution time = %ВремяВыполнения% sec)'");
...
Процедура ЧтениеФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		// АПК:1348-вкл
	Иначе
		СообщитьПользователю(НСтр("ru = 'Без расширения для работы с 1С:Предприятием невозможно сохранять и загружать запросы в файл.';
									|en = 'To save and import queries to files, install 1C:Enterprise Extension.'"), "Объект");
	КонецЕсли;
	...
	Для каждого СчитанныйПараметр Из РезультатСтруктура Цикл
		...
		Если Не НайденПараметр Тогда
			Если Не ФлагВыводаСообщения Тогда
				СообщитьПользователю(НСтр("ru = 'Найденные параметры были добавлены автоматически.';
											|en = 'Found parameters were added automatically.'"), "Объект");
...
Процедура ВключитьПланВыполненияЗапросаКлиент()
	...
	Если РезультатВключения.Результат Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Показывать план выполнения запроса';
											|en = 'Show query plan'"), , НСтр("ru = 'Включение анализа плана выполнения запроса занимает до одной минуты';
