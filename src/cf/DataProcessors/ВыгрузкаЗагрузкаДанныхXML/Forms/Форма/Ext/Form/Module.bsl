...
Процедура ВыгрузитьДанные(Команда)
	...
	Если РежимРаботыНаКлиенте И Не ПустаяСтрока(АдресФайлаВоВременномХранилище) Тогда
		...
		ИмяФайла = ?(Объект.ИспользоватьФорматFastInfoSet, НСтр("ru = 'Файл выгрузки.fi';
																|en = 'Export file.fi'"), НСтр("ru = 'Файл выгрузки.xml';
																								|en = 'Export file.xml'"));
...
Процедура ПроверкаСуществованияФайлаДанныхЗавершение(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		СообщитьПользователю(НСтр("ru = 'Файл не существует';
									|en = 'File does not exist'"), "ИмяФайлаЗагрузки");
		Возврат;
	КонецЕсли;
