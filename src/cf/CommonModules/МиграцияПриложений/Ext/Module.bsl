...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Исключение
		...
		Шаблон = НСтр("ru = 'Не удалось получить состояние загрузки, возможно сервис временно недоступен: %1 %2';
						|en = 'Cannot get the import state. The service might be temporarily unavailable: %1 %2'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьДанныеИзмененныхОбъектов(Узел) Экспорт
	...
	Запрос.Текст = ТекстЗапросаИзмененныхОбъектов();
...
Функция КоличествоИзмененных(Узел) Экспорт
	...
	Запрос.Текст = СтрШаблон(
		"ВЫБРАТЬ СУММА(КоличествоОбъектов) КАК Количество ИЗ (%1) КАК Т",
		ТекстЗапросаИзмененныхОбъектов());
...
// Текст запроса измененных объектов.
// 
// Возвращаемое значение:
//  Строка - Текст запроса измененных объектов
Функция ТекстЗапросаИзмененныхОбъектов()
