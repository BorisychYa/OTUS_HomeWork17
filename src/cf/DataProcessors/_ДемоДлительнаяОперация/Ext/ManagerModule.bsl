...
Функция РассчитатьЗначение(НомерОперации, ДлительностьРасчета = 10, ЗавершитьСОшибкой = Ложь, ВыводитьПрогрессВыполнения = Ложь) Экспорт
	...
	Если ЗавершитьСОшибкой Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка: расчет %1 не выполнен';
				|en = 'Error: calculation %1 is not completed'"), НомерОперации);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Расчет %1 выполнен за %2 сек.';
			|en = 'Calculation %1 is completed in %2 seconds'"), НомерОперации, ДлительностьРасчета);
...
Процедура ВыполнитьРасчет(НомерОперации, ДлительностьРасчета = 10, ЗавершитьСОшибкой = Ложь, ВыводитьПрогрессВыполнения = Ложь) Экспорт
	...
	Если ЗавершитьСОшибкой Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка: действие %1 не выполнено';
				|en = 'Error: action %1 is not completed'"), НомерОперации);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
