...
Процедура ВывестиРезультат(Результат, НомерВыполненнойОперации, ЭтоРасчетЗначения)
	...
	Если ЭтоРасчетЗначения Тогда
		...
		Если ТекстСообщения = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Из временного хранилища не получен результат (получено Неопределено)';
								|en = 'Из временного хранилища не получен результат (получено Неопределено)'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Действие %1 успешно выполнено';
				|en = 'Action %1 is completed'"), НомерВыполненнойОперации);
	КонецЕсли;
	...
	Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 %2';
			|en = '%1 %2'"), ТекущаяДатаСеанса(), ТекстСообщения);
	Сообщение.Сообщить();
