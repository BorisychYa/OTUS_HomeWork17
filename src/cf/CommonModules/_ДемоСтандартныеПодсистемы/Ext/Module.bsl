...
Процедура ПроверитьКомментарийВПоступленииТоваров(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока Результат.Следующий() Цикл
		...
		Проблема.УточнениеПроблемы = ?(ЗначениеЗаполнено(Результат.Комментарий), НСтр("ru = 'В комментарии введены пробелы или табуляции.';
																						|en = 'There are spaces or tabulations in the comment.'"), НСтр("ru = 'Не введен комментарий в документе.';
																																					|en = 'A comment is not entered in the document.'"));
...
Процедура ПриНачальномЗаполненииРолейИсполнителей(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.КраткоеПредставление             = НСтр("ru = '000000002';
													|en = '000000002'", ОбщегоНазначения.КодОсновногоЯзыка());
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, "Наименование",
		"ru = 'Демо: Руководитель компании';
		|en = 'Demo: CEO'", КодыЯзыков); // @НСтр-1
	...
	Элемент.КраткоеПредставление             = НСтр("ru = '000000003';
													|en = '000000003'", ОбщегоНазначения.КодОсновногоЯзыка());
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, "Наименование",
		"ru = 'Демо: Руководитель подразделения';
		|en = 'Demo: Department manager'", КодыЯзыков); // @НСтр-1
	...
	Элемент.КраткоеПредставление             = НСтр("ru = '000000004';
													|en = '000000004'", ОбщегоНазначения.КодОсновногоЯзыка());
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, "Наименование",
		"ru = 'Демо: Руководитель проекта';
		|en = 'Demo: Project manager'", КодыЯзыков); // @НСтр-1
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо"
		...
		ИЛИ НазначениеШаблона = "Документ._ДемоЗаказПокупателя") Тогда
			...
			Если ПараметрыШаблона.ФорматПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML Тогда
				...
				КонецЕсли;
			Иначе
				Сообщение.ЗначенияРеквизитов["КнопкаДляОплатыЯндексКасса"] = НСтр("ru = 'Оплатить счет:';
																					|en = 'Pay the bill:'") + Символы.ПС + Ссылка;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриПроверкеДоверенностиПодписи(ДанныеДоверенности, Сертификат, ПротоколПроверки) Экспорт
	...
	Иначе
		...
		ПроверкаДокумента.ТекстПроверки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Демо: Организация %1 (ИНН %2, КПП %3) не соответствует доверенности';
				|en = 'Demo: The %1 company (TIN %2, KPP %3) does not match the letter of authority'"), Организация, ИНН, КПП);
	КонецЕсли;
	...
	ПроверкаПолномочий.ЗаголовокПроверки = НСтр("ru = 'Проверка полномочий на подписание счета на оплату';
												|en = 'Check for powers to sign proforma invoice'");
	...
	Иначе
		...
		ПроверкаПолномочий.ТекстПроверки = НСтр("ru = 'Демо: Доверенность не предназначена для подписания счета на оплату';
												|en = 'Demo: You cannot use the letter of authority to sign the proforma invoice'");
	КонецЕсли;
...
Процедура ПриПолученииСтатусаРегистрацииМЧД(Доверенность, РегистрацияВРеестре) Экспорт
	...
	РегистрацияВРеестре.ЗаголовокФлажкаРегистрации = НСтр("ru = 'Регистрировать в реестре ФТС';
															|en = 'Register in the FCS registry'");
		...
		Если ТекущаяДатаСеанса() - ВремяРегистрации < 10 Тогда
			РегистрацияВРеестре.СостояниеРегистрации = СтроковыеФункции.ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<span style=""color: %1"">Отправлена на регистрацию в реестр ФТС %2</span>';
						|en = '<span style=""color: %1"">Sent for registration to the FCS registry %2</span>'"), "ПоясняющийТекст", ВремяРегистрации));
			...
			РегистрацияВРеестре.ПодсказкаСостояния = НСтр("ru = 'Запрос на регистрацию отправлен в реестр ФТС, ожидается результат запроса';
															|en = 'The registration request is sent to the FCS registry. The request result is expected.'");
			
		Иначе
			РегистрацияВРеестре.СостояниеРегистрации = СтроковыеФункции.ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<span style=""color: %1"">Зарегистрирована в реестре ФТС</span>';
						|en = '<span style=""color: %1"">Registered in the FCS registry</span>'"), "ПоясняющийТекст"));
			РегистрацияВРеестре.ЗаголовокФлажкаРегистрации = НСтр("ru = 'Зарегистрирована в реестре ФТС';
																	|en = 'Registered in the FCS registry'");
	...
	Иначе
		...
		РегистрацияВРеестре.ПодсказкаРегистрации = НСтр("ru = 'Если предполагается использовать доверенность для подписания документов, предоставляемых в таможенные органы, ее надо регистрировать в реестре ФТС.';
														|en = 'If you intend to use the letter of authority for signing documents submitted to customs authorities, register it in the Federal Customs Service registry.'");
	КонецЕсли;
...
Процедура ЗаполнитьПрофильПользователь(Знач ОписанияПрофилей)
	...
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Пользователь';
										|en = 'User'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьПрофильКладовщик(Знач ОписанияПрофилей)
	...
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Кладовщик';
										|en = 'Warehouse supervisor'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиля.Описание =
		НСтр("ru = 'Предназначен для настройки групп доступа, демонстрирующих
		           |работу пользователей по вводу и изменению данных различных подсистем в разрезе различных
		           |видов доступа. В частности:
...
Процедура ЗаполнитьПрофильРуководитель(Знач ОписанияПрофилей)
	...
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Руководитель';
										|en = 'Senior manager'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиля.Описание =
...
Процедура ЗаполнитьПрофильАудитор(Знач ОписанияПрофилей)
	...
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Аудитор';
										|en = 'Auditor'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьПрофильПартнер(Знач ОписанияПрофилей)
	...
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Партнер';
										|en = 'Partner'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьПрофильПечатьНепроведенныхДокументов(Знач ОписанияПрофилей)
	...
	ОписаниеПрофиля.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Дополнительно назначается тем пользователям, которым должна быть
		           ...
		           |It demonstrates the use of %1 function.'"), "ЕстьРоль");
